...
namespace
{
	...
	// Object references intermediate path segments may hop across — mirrors the
	// runtime rule in ConvaiPropertyReflection.cpp (private, hence the local
	// copy — keep the two in sync): hard/weak refs to sub-actors, components,
	// or instanced subobjects. Never bindable as a leaf; only drill-down.
	bool IsConvaiTraversableObjectProperty(const FProperty* Prop)
	{
	...
	IPropertyTypeCustomizationUtils& /*StructCustomizationUtils*/)
{
	...
	const TSharedPtr<IPropertyHandle> ShouldRespondHandle =
		StructPropertyHandle->GetChildHandle(
			GET_MEMBER_NAME_CHECKED(FConvaiTrackedProperty, ShouldRespond));
	...
	for (uint32 i = 0; i < NumChildren; ++i)
	{
		...
		// Internal bookkeeping — never shown in the details panel.
		if (ChildName == GET_MEMBER_NAME_CHECKED(FConvaiTrackedProperty, PropertyPathGuids))
		{
			continue;
		}
		...
		else
		{
			...
			if (ChildName == GET_MEMBER_NAME_CHECKED(FConvaiTrackedProperty, Delivery)
				|| ChildName == GET_MEMBER_NAME_CHECKED(
					FConvaiTrackedProperty, bFlushImmediately))
			{
				const TWeakPtr<IPropertyHandle> WeakShouldRespond = ShouldRespondHandle;
				Row.IsEnabled(TAttribute<bool>::CreateLambda(
					[WeakShouldRespond]()
					{
						const TSharedPtr<IPropertyHandle> Handle =
							WeakShouldRespond.Pin();
						if (!Handle.IsValid())
						{
							return true;
						}
						uint8 Value = static_cast<uint8>(EC_RunLLMOption::Never);
						const FPropertyAccess::Result Result = Handle->GetValue(Value);
						return Result != FPropertyAccess::Success
							|| static_cast<EC_RunLLMOption>(Value)
...
TSharedRef<SWidget> FConvaiTrackedPropertyCustomization::MakeBindingWidget(UClass* OwnerClass)
{
	...
	Args.bAllowUObjectFunctions        = true;  // pure functions on hopped-to sub-actors / components too
	Args.bAllowStructFunctions         = false;
	Args.bGeneratePureBindings         = true;  // only show pure / const functions

	// Interfaces / classes / delegates and their subtrees stay closed, but object
	// references to sub-actors / components / instanced subobjects
	// are offered as DRILL-DOWN (runtime hops across them with IsValid checks
	// every poll) — they're still not bindable as leaves. Structs are walked
	// into; the "no struct leaf" rule below just suppresses the duplicate "bind
	// whole struct" entry next to the submenu (FVector / FRotator / etc.
	// value-structs are excepted via IsConvaiBindableProperty).
	auto CanAcceptLambda = [](FProperty* InProperty) -> bool
	{
		return IsConvaiBindableProperty(InProperty) || IsConvaiTraversableObjectProperty(InProperty);
	};
	...
	Args.OnCanBindFunction = FOnCanBindFunction::CreateLambda(
		[](UFunction* InFunction) -> bool
		{
			...
			// Purity is enforced here (not just via bGeneratePureBindings) so the
			// picker can never offer a function the runtime resolver would reject.
	...
	// Setting the path fires the component's PostEditChangeChainProperty, whose
	// heal pass records the per-segment rename GUIDs (PropertyPathGuids) — no
	// GUID capture needed here.
	...
	Args.OnRemoveBinding = FOnRemoveBinding::CreateLambda(
		...
		{
			// Stale PropertyPathGuids are harmless — the heal pass resizes them
			// to the next bound path.
