...
public:
	...
	/** How many times the plugin reopens a session that dropped without being
	 *  asked to. 0 turns reconnect off entirely.
	 *
	 *  One knob rather than a switch and a count, because two cannot disagree
	 *  with each other. It counts attempts per drop, not per session: a drop
	 *  that recovers resets it, so a long-running game is not slowly spending a
	 *  lifetime budget.
	 *
	 *  Defaults to 0 in this release: the plugin behaves as it always has until
	 *  a project opts in. */
	UPROPERTY(Config, EditAnywhere, Category = "Connection",
	          meta = (DisplayName = "Reconnect Attempts", ClampMin = "0", ClampMax = "10", UIMin = "0", UIMax = "10"))
	int32 ReconnectAttempts = 0;

	/** What a session the server closed for inactivity does next. Reopen When
	 *  Player Returns holds nothing open while nobody is there and reopens the
	 *  moment the player speaks, types or presses to talk; Reopen Immediately
	 *  reopens at once (once per idle close) and holds a concurrency slot;
	 *  Stay Closed ends the session for good.
	 *
	 *  Ignored while Reconnect Attempts is 0: that is today's behaviour, one
	 *  Disconnected and nothing reopened. */
	UPROPERTY(Config, EditAnywhere, Category = "Connection",
	          meta = (DisplayName = "Idle Timeout Behavior"))
	EC_IdleTimeoutBehavior IdleTimeoutBehavior = EC_IdleTimeoutBehavior::ReopenWhenPlayerReturns;
	...
	// ── Scene Auto Tagger ──────────────────────────────────────────────
	// The editor-only workflow that looks at your level through Convai Vision
	// and proposes Convai Object names and descriptions. Capture, discovery,
	// and review preferences live in their own per-user page under
	// Plugins ▸ Convai Scene Auto Tagger; this is the account-level identity
	// the workflow bills and authorizes its Vision requests with.

	/** Character whose account authorizes Scene Auto Tagger's Convai Vision
	 *  requests. While empty the workflow falls back to the advanced debug Test
	 *  Character ID above, so projects configured before this setting existed
	 *  keep working. Starting the editor with -ConvaiVisionCharacterID=<id>
	 *  overrides both for that session. */
	UPROPERTY(Config, EditAnywhere, Category = "Scene Auto Tagger",
	          meta = (DisplayName = "Vision Character ID"))
