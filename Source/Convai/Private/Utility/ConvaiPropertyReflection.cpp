...
bool ConvaiPropertyReflection::ResolvePath(UObject* Root, FName DottedPath, FConvaiResolvedProperty& Out)
{
	...
	// Single token: try a UFunction on the owner first (functions live on UClass, not
	// inside structs). The picker only allows pure, parameter-less functions whose
	// return is a supported leaf type — runtime mirrors that check as defense in depth
	// (a user could otherwise hand-type a non-pure function name and have it invoked
	// every poll tick).
	if (Tokens.Num() == 1)
	{
		if (UFunction* Func = Root->FindFunction(FName(*Tokens[0])))
		{
			const bool bIsPure = Func->HasAnyFunctionFlags(FUNC_BlueprintPure | FUNC_Const);
			if (bIsPure && IsSupportedLeaf(Func->GetReturnProperty()))
			{
				Out.LeafFunction = Func;
				Out.FunctionTarget = Root;
				Out.bSupported = true;
				return true;
			}
		}
	...
	// Property walk: arbitrary depth through struct members.
	...
	{
		FProperty* Found = CurrentStruct ? CurrentStruct->FindPropertyByName(FName(*Tokens[i])) : nullptr;
		if (!Found)
		{
			return false;
		}
		...
		if (i + 1 < Tokens.Num())
		{
			FStructProperty* StructProp = CastField<FStructProperty>(Found);
	...
	Out.Leaf = CurrentProp;
	Out.LeafContainer = CurrentContainer;
	Out.bSupported = IsSupportedLeaf(CurrentProp);
	return true;
}
