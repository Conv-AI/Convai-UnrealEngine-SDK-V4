...
public:
     ...
     * The ticker starts on first registration, stops on last unregistration,
     * and invokes EvaluateTrackedProperties() on every registered object in
     * the same tick instant so the chatbot's ContextDebounceWindow can
...
private:
    ...
    // Shared poll clock state for ConvaiObjectComponents.
    ...
    float ComputeEffectiveObjectPollInterval();

    /** (Re)starts the shared object-poll ticker with the current effective interval. Idempotent. */
