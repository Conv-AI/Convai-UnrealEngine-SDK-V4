...
void FWelcomePageViewModel::Initialize()
{
    ...
    FConvaiValidationUtils::ResolveServiceWithCallbacks<IWelcomeService>(
        ...
        [this](TSharedPtr<IWelcomeService> Service)
        {
            ...
            {
                OnApiKeyValidatedHandle = ServicePtr->OnApiKeyValidated().AddSP(this, &FWelcomePageViewModel::OnApiKeyValidated);
                OnApiKeyValidationFailedHandle = ServicePtr->OnApiKeyValidationFailed().AddSP(this, &FWelcomePageViewModel::OnApiKeyValidationFailed);

                TrackDelegateHandle(OnApiKeyValidatedHandle);
                TrackDelegateHandle(OnApiKeyValidationFailedHandle);
            }
        },
        [](const FString &Error)
        {
...
void FWelcomePageViewModel::Shutdown()
{
    ...
    {
        if (OnApiKeyValidatedHandle.IsValid())
        {
            Service->OnApiKeyValidated().Remove(OnApiKeyValidatedHandle);
            OnApiKeyValidatedHandle.Reset();
        }

        if (OnApiKeyValidationFailedHandle.IsValid())
        {
            Service->OnApiKeyValidationFailed().Remove(OnApiKeyValidationFailedHandle);
            OnApiKeyValidationFailedHandle.Reset();
        }
    }
