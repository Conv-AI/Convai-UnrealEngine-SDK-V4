...
public:
	struct FConfig
	{
		...
		// Defaults are set via the ctor body, NOT via NSDMI. Clang 20 (UE 5.8)
		// rejects NSDMIs on a nested struct when the enclosing class declares
		// a method whose default arg constructs that struct — it tries to
		// materialize the NSDMIs at a point where the outer class isn't complete.
		FConfig()
			: ContentType(EContentFeedCacheType::Announcements)
			, TTLSeconds(3600.0)
			, CacheFileName(TEXT("content_feed_cache.json"))
			, bEnableDiskCache(true)
		{}
	};
	 ...
	 *
	 * Uses `= {}` (not `= FConfig()`) as the default so clang 20 (UE 5.8) doesn't
	 * try to instantiate FConfig's default member initializers while still inside
	 * the enclosing class — that triggers "default member initializer needed
	 * within definition of enclosing class outside of member functions".
	 */
	explicit FContentFeedCacheManager(const FConfig &InConfig = {});
