...
uint32 FConvaiConnectionThread::Run()
{
    ...
    {
        if (!ConnectionParams.Client->Initialize(UConvaiUtils::IsAECEnabled(), UConvaiUtils::IsVADEnabled()))
        {
        ...
        if (StreamURLLen < 0 || AuthKeyHeaderLen < 0 || AuthKeyValueLen < 0 || CharIDLen < 0 || ConnectionTypeLen < 0 || LLMProviderLen < 0 || BlendshapeProviderLen < 0)
        {
        ...
        if (!ConnectionParams.Client->Connect(StreamURLBuffer, AuthKeyValueBuffer, AuthKeyHeaderBuffer, CharIDBuffer, ConnectionTypeBuffer, LLMProviderBuffer, BlendshapeProviderBuffer))
        {
...
bool UConvaiSubsystem::ConnectSession(UConvaiConnectionSessionProxy* SessionProxy, const FString& CharacterID)
{
    ...
    if (ConvaiClient)
    {
        ...
        FConvaiConnectionParams ConnectionParams = FConvaiConnectionParams::Create(ConvaiClient, CharacterID, SessionProxy);
...
bool UConvaiSubsystem::InitializeConvaiClient()
{
    ...
    ConvaiClient = new convai::ConvaiClient(nullptr, false);
...
void UConvaiSubsystem::OnConnectedToServer()
{
    ...
    if (!ReferenceAudioThread.IsValid() && UConvaiUtils::IsAECEnabled())
    {
        ...
        {
            ReferenceAudioThread = MakeShared<FConvaiReferenceAudioThread>(ConvaiClient, World);
...
void UConvaiSubsystem::OnDisconnectedFromServer()
{
    ...
    // Stop reference audio capture when disconnected
    if (ReferenceAudioThread.IsValid())
    {
        ReferenceAudioThread->StopCapture();
...
void UConvaiSubsystem::StartTestRecording()
{
    
}

void UConvaiSubsystem::StopTestRecording()
{
