...
void UConvaiChatbotComponent::AddContextEvent(const FString& Text, EC_RunLLMOption ShouldRespond, bool bEphemeral, bool bFlushImmediately)
{
...
void UConvaiChatbotComponent::SetContextFact(const FString& Sentence, const FString& Key, EC_RunLLMOption ShouldRespond, bool bFlushImmediately)
{
...
void UConvaiChatbotComponent::FlushDynamicContext()
{
	...
	if (bHasStagedBatch || bHasAttention)
	{
		...
		// Declarative-fact delta lines are the bare sentence (the fact itself);
		// new vs changed reads identically, so we don't special-case "changed to".
		for (const FString& Key : PendingContextBatch.StagedDeclarativeOrder)
		{
			FString CurrentSentence;
			DynamicContextTracker.GetDeclarativeValue(Key, CurrentSentence);
			DeltaLines.Add(CurrentSentence);
		}
		...
		// Declarative facts: defer ALL staged ones (new AND changed) from canonical
		// when we're emitting deltas, because a declarative's canonical line and its
		// delta line are the identical sentence — keeping both would print the fact
		// twice. With Aggregate == Never there is no delta block, so they stay.
		TSet<FString> DeferredDeclarativeKeys;
		if (PendingContextBatch.AggregateRunLLM != EC_RunLLMOption::Never)
		{
			DeferredDeclarativeKeys.Append(PendingContextBatch.StagedDeclarativeOrder);
		}

		FString Canonical = DynamicContextTracker.BuildCanonicalContext(FirstAppearanceKeys, DeferredDeclarativeKeys);
...
void UConvaiChatbotComponent::SetObjectInAttention(FConvaiObjectEntry AttentionObject,
	FString Text, EC_RunLLMOption ShouldRespond, bool bAddAttentionEvent, bool bFlushImmediately)
{
	...
	// 3. Stage the attention slot, and (by default) a ONE-FLUSH event announcing the focus.
	...
	FString Text, EC_RunLLMOption ShouldRespond, bool bAddAttentionEvent, bool bFlushImmediately)
{
	...
	SetObjectInAttention(AttentionObject, Text, ShouldRespond, bAddAttentionEvent, bFlushImmediately);
...
bool UConvaiChatbotComponent::TryClearObjectInAttentionFromGaze(FConvaiObjectEntry ExpectedObject)
{
	...
	SetObjectInAttention(FConvaiObjectEntry(), TEXT(""), EC_RunLLMOption::Never, /*bAddAttentionEvent=*/ false, /*bFlushImmediately=*/ false);
	return true;
}
