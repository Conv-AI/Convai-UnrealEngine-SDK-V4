...
public:
	...
	*/
	 ...
	 * @param RunLLM         Whether to trigger an LLM response: Auto, True, or False
	 */
	UFUNCTION(BlueprintCallable, Category = "Convai")
	void UpdateContext(const FString& Text, EC_ContextUpdateMode Mode = EC_ContextUpdateMode::Append, EC_RunLLMOption RunLLM = EC_RunLLMOption::Auto);
	 ...
	 *
	 * With RunLLM = Never: rebuilds and replaces the full canonical context.
	 * With RunLLM = Auto/Always: also appends a change message to trigger the LLM.
	 */
	UFUNCTION(BlueprintCallable, Category = "Convai|DynamicContext", meta = (DisplayName = "Set Context State"))
	void SetContextState(const FString& Name, const FString& Value, EC_RunLLMOption RunLLM = EC_RunLLMOption::Never);

	/**
	 * Sets multiple state properties at once. When RunLLM is Auto/Always, the
	 * change messages are combined into a single append so only one LLM response
	 * is triggered.
	 */
	UFUNCTION(BlueprintCallable, Category = "Convai|DynamicContext", meta = (DisplayName = "Set Context States"))
	void SetContextStates(const TMap<FString, FString>& States, EC_RunLLMOption RunLLM = EC_RunLLMOption::Never);

	/**
	 * Adds a chronological event to the dynamic context (e.g. "Player approached the merchant").
	 * The event is appended directly using the provided RunLLM option.
	 */
	UFUNCTION(BlueprintCallable, Category = "Convai|DynamicContext", meta = (DisplayName = "Add Context Event"))
	void AddContextEvent(const FString& Text, EC_RunLLMOption RunLLM = EC_RunLLMOption::Auto);

	/**
	 * Removes a state property from the dynamic context and rebuilds the canonical context.
	 */
	UFUNCTION(BlueprintCallable, Category = "Convai|DynamicContext", meta = (DisplayName = "Remove Context State"))
	void RemoveContextState(const FString& Name);
