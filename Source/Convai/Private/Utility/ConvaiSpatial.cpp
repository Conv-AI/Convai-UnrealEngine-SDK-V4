...
namespace ConvaiSpatial
{
	...
	{
		if (Dir.bColocated) { return TEXT("right there"); }

		TArray<FString, TInlineAllocator<3>> Parts;
		if (Dir.Forward == EForwardDir::Front)       { Parts.Add(TEXT("ahead")); }
		else if (Dir.Forward == EForwardDir::Behind) { Parts.Add(TEXT("behind")); }
		if (Dir.Lateral == ELateralDir::Right)       { Parts.Add(TEXT("to the right")); }
		else if (Dir.Lateral == ELateralDir::Left)   { Parts.Add(TEXT("to the left")); }
		if (Dir.Vertical == EVerticalDir::Above)     { Parts.Add(TEXT("above")); }
		else if (Dir.Vertical == EVerticalDir::Below){ Parts.Add(TEXT("below")); }

		return JoinParts(Parts, TEXT("right there"));
	}
	...
	FString BandWord(EProximityBand Band)
	{
		switch (Band)
		{
			case EProximityBand::Nearby:   return TEXT("close by");
			case EProximityBand::Moderate: return TEXT("some distance away");
			default:                       return TEXT("far away");
		}
	}

	FString BandPhrase(EProximityBand Band, const FString& Direction)
	{
		// Unreachable leads with the direction: the exact distance is moot when
		// there is no path, and BandWord(Unreachable) intentionally reads "far
		// away" for callers without nav data.
		if (Band == EProximityBand::Unreachable)
		{
			return FString::Printf(TEXT("%s, with no walking path there"), *Direction);
		}
		return FString::Printf(TEXT("%s, %s, reachable by walking"),
			*BandWord(Band), *Direction);
	}
