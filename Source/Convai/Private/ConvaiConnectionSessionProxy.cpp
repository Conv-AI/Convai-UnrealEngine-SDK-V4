...
void UConvaiConnectionSessionProxy::ForceUserStoppedSpeaking() const
{
    if (const UConvaiSubsystem* ConvaiSubsystem = UConvaiUtils::GetConvaiSubsystem(this))
    {
        ConvaiSubsystem->ForceUserStoppedSpeaking(this);
    }
}

void UConvaiConnectionSessionProxy::UpdateContext(const FString& Text, EC_ContextUpdateMode Mode, EC_RunLLMOption ShouldRespond,
    const FConvaiObjectEntry* OptionalAttention) const
{
    if (const UConvaiSubsystem* ConvaiSubsystem = UConvaiUtils::GetConvaiSubsystem(this))
    {
        ConvaiSubsystem->UpdateContext(this, Text, Mode, ShouldRespond, OptionalAttention);
    }
}
