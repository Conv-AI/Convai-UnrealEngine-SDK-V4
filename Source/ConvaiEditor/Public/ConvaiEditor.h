...
public:
    ...
    /** Opens the general Convai Scene Objects workspace. */
...
private:
    /** Registers the Movement Points viewport visualizer for Convai Object components (defers to OnPostEngineInit when GUnrealEd isn't up yet) */
    void RegisterMovementPointVisualizer();
    ...
    /** Builds the general Scene Objects workspace tab. */
    TSharedRef<class SDockTab> OnSpawnSceneObjectsTab(const class FSpawnTabArgs &SpawnTabArgs);
    ...
    FDelegateHandle MovementPointVisualizerInitHandle;
    std::atomic<bool> bEarlyCleanupCompleted{false};
    bool bStyleSystemInitialized = false;
};
