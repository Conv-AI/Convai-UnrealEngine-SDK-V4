...
void UConvaiAudioStreamer::ProcessIncomingAudio(bool Force)
{
	...
	if (!bIsPlayingAudio)
	{
		...
		if (SupportsLipSync())
		{
			UE_LOG(ConvaiAudioStreamerLog, Log, TEXT("[AudioStreamer] ProcessIncomingAudio - Starting Lipsync"));
...
void UConvaiAudioStreamer::MarkEndOfAudio()
{
	...
	if (TotalAudioBytesReceived > 0 && LastReceivedSampleRate > 0 && LastReceivedNumChannels > 0)
	{
		...
		UE_LOG(ConvaiAudioStreamerLog, Log, TEXT("[AudioStreamer] MarkEndOfAudio - Total audio received from WebRTC: %u bytes, %.3f seconds (SR: %u, CH: %u)"),
