...
void FBrushTokensLoader::ProcessBrushesObject(const TSharedPtr<FJsonObject> &JsonObj, TSharedPtr<FSlateStyleSet> Style, const FString &ResourceRootPath)
{
    ...
    {
        // 5.7 FString / 5.8 TSharedString bridge via TCHAR*. Can't bind a const
        // reference to a temporary FString that would be constructed from the key.
        ...
        if (Value->Type == EJson::String)
        {
            ...
            else if (BrushValue.Contains(TEXT("/")))
            {
                ...
                FName StyleKey = FName(*Key);
                if (FPaths::GetExtension(BrushValue).Equals(TEXT("svg"), ESearchCase::IgnoreCase))
                {
                    Style->Set(StyleKey, new FSlateVectorImageBrush(FullImagePath, ImageSize));
                }
                else
                {
                    Style->Set(StyleKey, new FSlateImageBrush(FullImagePath, ImageSize));
                }
            }
            else
            {
...
void FBrushTokensLoader::GenerateSurfaceBrushes(const TSharedPtr<FJsonObject> &ColorTokens, TSharedPtr<FSlateStyleSet> Style)
{
    ...
    if (ColorTokens->TryGetObjectField(TEXT("surface"), SurfaceObject) && SurfaceObject)
    {
        ...
        {
            const FString ColorKey(*Pair.Key); // 5.7/5.8 key-type bridge
...
void FBrushTokensLoader::ProcessComponentBrushes(const TSharedPtr<FJsonObject> &ComponentObject, TSharedPtr<FSlateStyleSet> Style)
{
    ...
    {
        const FString GroupKey(*GroupPair.Key); // 5.7/5.8 key-type bridge
        ...
        if (GroupValue->Type == EJson::Object)
        {
            ...
            {
                const FString ColorKey(*ColorPair.Key); // 5.7/5.8 key-type bridge
                ...
                if (ColorValue->Type == EJson::Object)
                {
                    ...
                    {
                        const FString NestedKey(*NestedPair.Key); // 5.7/5.8 key-type bridge
