...
// Engine/TimerHandle.h is split out in UE 5.1+; older versions expose
// FTimerHandle via TimerManager.h.
...
public:
	...
	/**
	 * Test chunked audio streaming with regular intervals (similar to replay component).
	 * Sends audio chunks of specified duration at regular intervals until all audio is sent.
	 *
	 * @param AudioStreamer			The audio streamer to send chunks to
	 * @param AudioToPlay			The source audio to stream
	 * @param ChunkDurationSeconds	Duration of each audio chunk in seconds (e.g., 0.1 for 100ms chunks)
	 * @param IntervalSeconds		Time interval between sending chunks in seconds (e.g., 0.1 to send every 100ms)
	 */
	UFUNCTION(BlueprintCallable, meta = (BlueprintInternalUseOnly = "true", DisplayName = "Test Chunked Audio Streaming (Interval)", WorldContext = "WorldContextObject"), Category = "Convai|Debug|Tests")
	static UConvaiAudioChunkTestProxy* CreateAudioChunkTestProxyWithInterval(
		UObject* WorldContextObject,
		UConvaiAudioStreamer* AudioStreamer,
		USoundWave* AudioToPlay,
		float ChunkDurationSeconds = 0.1f,
