...
// do/while(0) so the two statements are one statement. Without it a brace-less
// `if (x) CONVAI_LOG(...)` guards the UE_LOG and leaves the file log running
// unconditionally -- which is what crashed teardown on most launches for months:
// it evaluated OutSoundWave->GetDuration() on the null pointer the `if` had just
// rejected (ISSUES I7). Arguments are still evaluated twice by design, so a call
// whose argument has a side effect sees it twice.
...
// Runtime-category variant: use when the category is a FLogCategoryBase& parameter, not a symbol.
#define CONVAI_LOG_REF(CategoryRef, Verbosity, Format, ...)             \
do {                  \
UE_LOG_REF(CategoryRef, Verbosity, Format, ##__VA_ARGS__);              \
FConvaiLogger::Get().Log(                                               \
FString::Printf(                                                        \
TEXT("%s : ") TEXT(#Verbosity) TEXT(" : ") Format,                      \
*(CategoryRef).GetCategoryName().ToString(),                            \
##__VA_ARGS__                                                           \
)                                                                       \
);                    \
} while (0) ;
