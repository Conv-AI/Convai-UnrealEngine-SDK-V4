...
public:
	...
	/** Character authorizing Convai Vision requests; falls back to the test character while unset. */
	UFUNCTION(BlueprintPure, Category = "Convai|Settings")
	static FString GetVisionCharacterID();
	...
	/** "Listener" (a submix tap) or "Recorder" (the output recorder). */
	static FString GetReferenceCaptureTap();
	static FString GetMicCaptureTap();
	/** Empty means never call SetStreamDelay; "0" is a real hint of zero. */
	static FString GetAECStreamDelayMs();
	...
	UFUNCTION(BlueprintPure, Category = "Convai|Utilities")
	/** Project Settings ▸ Plugins ▸ Convai ▸ Reconnect Attempts, clamped 0-10.
	 *  Overridable with -ReconnectAttempts= for tests. */
	static int32 GetReconnectAttempts();

	/** Project Settings ▸ Plugins ▸ Convai ▸ Idle Timeout Behavior. Overridable
	 *  with -IdleTimeoutBehavior=ReopenWhenPlayerReturns|ReopenImmediately|StayClosed
	 *  for tests. */
	static EC_IdleTimeoutBehavior GetIdleTimeoutBehavior();
	 ...
	 * @param bRequireAnimInstance When true (default), meshes without a running anim
	 *                             instance (no AnimBP) are excluded from the candidate
	 *                             set unless no mesh has one.
	 */
	UFUNCTION(BlueprintCallable, Category = "Convai|Utilities")
	static void GetBodyAndFaceSkeletalMeshComponents(AActor* Actor, class USkeletalMeshComponent*& BodyComponent, class USkeletalMeshComponent*& FaceComponent, bool bRequireAnimInstance = true);
};
