...
{
#if ENGINE_MAJOR_VERSION >= 5 && ENGINE_MINOR_VERSION >= 4
	CachedSoundSource = nullptr;
	FramesPlayedAtSessionStart = 0;
#endif
...
void UConvaiAudioStreamer::PlayVoiceData(uint8* VoiceData, uint32 VoiceDataSize, bool ContainsHeaderData, uint32 SampleRate, uint32 NumChannels)
{
    ...
    {
#if ENGINE_MAJOR_VERSION >= 5 && ENGINE_MINOR_VERSION >= 4
        // Cache the FSoundSource so GetAudioPlaybackTime() can read GetNumFramesPlayed()
        // directly on the game thread without per-tick async dispatch.
        CacheSoundSource();
#endif
        onAudioStarted();
        IsTalking = true;
...
void UConvaiAudioStreamer::HandleAudioReceived(uint8* AudioData, uint32 AudioDataSize, bool ContainsHeaderData, uint32 SampleRate, uint32 NumChannels)
{
	...
	if (SupportsLipSync() && !ConvaiLipSync->RequiresPrecomputedFaceData())
	{
		...
		LipSyncAudioRingBuffer.Enqueue(AudioData, AudioDataSize);
	}
...
void UConvaiAudioStreamer::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
#if ConvaiDebugMode && ENGINE_MAJOR_VERSION >= 5 && ENGINE_MINOR_VERSION >= 4
	if (IsTalking && CachedSoundSource && IsValid(SoundWaveProcedural) && AudioPlaybackSampleRate > 0)
	{
		static int32 DebugTickCount = 0;
		if (++DebugTickCount % 30 == 0)
		{
			int64 TotalFrames   = CachedSoundSource->GetNumFramesPlayed();
			double TimeA        = GetAudioPlaybackTime();
			...
			double TimeB        = static_cast<double>(BytesPlayed) / (static_cast<double>(AudioPlaybackSampleRate) * AudioPlaybackNumChannels * 2.0);
			UE_LOG(ConvaiAudioStreamerLog, Log,
				TEXT("[AudioDbg] TotalFrames=%lld | TimeA(frames)=%.3fs | Enqueued=%lld Avail=%d Played=%lld | TimeB(bytes)=%.3fs | Delta=%.3fs"),
				TotalFrames, TimeA, BytesEnqueued, BytesAvail, BytesPlayed, TimeB, TimeA - TimeB);
		}
	}
#endif
	...
	{
		if (GetRemainingContentDuration() <= 0 && AudioRingBuffer.IsEmpty())
		{
			onAudioFinished();
		}
	}
...
{
    if (!CachedSoundSource || AudioPlaybackSampleRate == 0)
        return 0.0;

    // GetNumFramesPlayed() is backed by an atomic counter in FMixerSourceManager and is safe
    // to read from the game thread. It only increments when the mixer actually consumes data,
    // so it stalls during buffer underflows and network gaps - giving us true playback time.
    int64 CurrentFrames = CachedSoundSource->GetNumFramesPlayed();
    int64 FramesPlayed = FMath::Max<int64>(0, CurrentFrames - FramesPlayedAtSessionStart);
    return static_cast<double>(FramesPlayed) / static_cast<double>(AudioPlaybackSampleRate);
}

void UConvaiAudioStreamer::CacheSoundSource(bool bIsRetry)
{
    if (!bIsRetry)
    {
        CachedSoundSource = nullptr;
    ...
    uint64 ComponentID = GetAudioComponentID();
    TWeakObjectPtr<UConvaiAudioStreamer> WeakThis(this);

    // SendCommandToActiveSounds dispatches to the audio thread automatically.
    // We read NumFramesPlayed there as a baseline offset, then store both the
    // pointer and baseline back on the game thread via RunCommandOnGameThread.
    AudioDevice->SendCommandToActiveSounds(ComponentID,
        [AudioDevice, WeakThis, bIsRetry](FActiveSound& ActiveSound)
        {
            FWaveInstance* WaveInstance = nullptr;
            for (const auto& Pair : ActiveSound.GetWaveInstances())
            {
                WaveInstance = Pair.Value;
                break;
            }

            FSoundSource* SoundSource = WaveInstance ? AudioDevice->GetSoundSource(WaveInstance) : nullptr;

            if (!SoundSource)
            {
                // Schedule a single retry on the game thread after 0.1s (non-blocking)
                if (!bIsRetry)
                {
                    FAudioThread::RunCommandOnGameThread([WeakThis]()
                    {
                        ...
                        if (UWorld* World = WeakThis->GetWorld())
                        {
                            ...
                            World->GetTimerManager().SetTimer(RetryHandle,
                                ...
                                {
                                    if (WeakThis.IsValid())
                                        WeakThis->CacheSoundSource(true);
                                }), 0.1f, false);
                        }
                    });
                }
            ...
            int64 BaselineFrames = SoundSource->GetNumFramesPlayed();

            FAudioThread::RunCommandOnGameThread([WeakThis, SoundSource, BaselineFrames]()
            {
                if (WeakThis.IsValid())
                {
                    WeakThis->CachedSoundSource = SoundSource;
                    WeakThis->FramesPlayedAtSessionStart = BaselineFrames;
                }
            });
        });
}

#else // UE < 5.4 fallback: byte-consumption tracking via SoundWaveProcedural
