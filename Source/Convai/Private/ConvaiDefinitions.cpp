	...
	TArray<FVector>& OutPathPoints)
{
	...
	if (SourceActor)
	{
		...
		//  - Step Onto Bounds ON: 2D distance from Source Actor to the goal
		//    CENTRE (the target's top-face centre) — "already there" means
		//    standing on it, within the raw AcceptanceRadius (no ×2, no 150
		//    floor) so a small trigger volume like a pressure plate reads false
		//    the moment the actor steps off.
		...
		if (bStepOntoBounds)
		{
			const float CentreDist = FVector::Dist2D(SourceLoc, OutGoalLocation);
			bHorizArrived = CentreDist <= AcceptanceRadius;
		}
		else
		{
