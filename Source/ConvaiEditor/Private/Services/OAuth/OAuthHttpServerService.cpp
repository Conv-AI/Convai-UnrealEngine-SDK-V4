...
bool FOAuthHttpServerService::StartServer(const TArray<int32> &PreferredPorts)
{
    ...
    auto BindControlRoute = [this](const TSharedPtr<IHttpRouter> &Router) -> bool
    {
        if (!Router.IsValid())
        {
            return false;
...
#endif
        return ControlRouteHandle.IsValid();
    };

    FHttpServerModule &HttpServerModule = FHttpServerModule::Get();

    // HTTPServer listeners are shared process-wide. Reuse the listener left by a
    // previous login instead of requiring the port to become free again.
    if (CurrentPort > 0)
    {
        TSharedPtr<IHttpRouter> Router = HttpServerModule.GetHttpRouter(CurrentPort);
        if (BindControlRoute(Router))
        {
            bIsRunning = true;
            return true;
        }
...
void FOAuthHttpServerService::StopServer()
{
    ...
    // Do not call StopAllListeners here. The HTTPServer module is shared with
    // Unreal's MCP server and other editor services. The listener remains
    // available for a future OAuth login and is released with the module.
}
