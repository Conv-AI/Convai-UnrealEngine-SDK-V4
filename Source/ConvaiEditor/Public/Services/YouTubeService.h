...
private:
    /** Fetches latest video from the stable YouTube channel feed. */
    void FetchLatestVideoFromRSSFeed(
    ...
    /** Builds a channel RSS feed URL from channel ID, handle, URL, or default config. */
    FString BuildChannelFeedURL(const FString &ChannelName) const;
    ...
    /** Parses latest video metadata from YouTube channel RSS feed XML. */
    bool ParseLatestVideoFromRSSFeed(const FString &XMLContent, FYouTubeVideoInfo &OutVideoInfo) const;

    /** Parses a single RSS entry into video metadata. */
    bool ParseVideoInfoFromRSSEntry(const FString &Entry, FYouTubeVideoInfo &OutVideoInfo) const;
    ...
    /** Finds first lockupViewModel object in parsed JSON tree. */
    bool FindFirstLockupViewModelJSON(const TSharedPtr<FJsonValue> &Value, TSharedPtr<FJsonObject> &OutRenderer) const;

    /** Extracts first string field value matching regex from a parsed JSON tree. */
    bool FindFirstStringMatchingRegexJSON(const TSharedPtr<FJsonValue> &Value, const FString &PatternText, FString &OutValue) const;
    ...
    /** Decodes XML/HTML entities commonly returned by YouTube feeds and pages. */
    FString DecodeHTMLEntities(const FString &Input) const;

    /** Returns true when the URL is a YouTube Shorts URL. */
    bool IsShortsURL(const FString &VideoURL) const;
