...
void FSceneAutoTaggerController::ScheduleNextCaptureBatch()
{
	...
	const TWeakPtr<FSceneAutoTaggerController> WeakSelfForCallback = AsShared();
	FTSTicker::GetCoreTicker().AddTicker(
		FTickerDelegate::CreateLambda([WeakSelfForCallback, CaptureRunId](float)
		{
			if (const TSharedPtr<FSceneAutoTaggerController> Self = WeakSelfForCallback.Pin())
			{
...
void FSceneAutoTaggerController::CaptureNextCell(FGuid CaptureRunId)
{
	...
	if (Candidate.IsValid())
	{
		...
		if (ActiveBatch->CurrentProbeLookDirections.IsEmpty())
		{
			...
			// are asynchronous; the winning 512px view below remains alive while their
			...
			&& !ActiveBatch->bCurrentProbeWarmupComplete)
		{
			...
			if (ConvaiSceneAutoTagger::ShouldContinueAutomaticProbeWarmup(
				ActiveBatch->CurrentProbeWarmupTicks,
				ElapsedSeconds,
				PendingRenderWork))
			{
			...
			UE_LOG(
				LogConvaiSceneAutoTaggerController,
				Verbose,
				TEXT("Probe warm-up '%s': frames=%d elapsed=%.3fs pendingAssets=%d compilingMaterials=%d."),
		...
		if (ActiveBatch->SelectedProbeIndex != INDEX_NONE)
		{
			...
			if (ActiveBatch->CurrentFinalCaptureSession)
			{
				...
				// 512px view alive until conventional assets settle, with bounded extra
		...
		else
		{
			...
			if (!ActiveBatch->bCurrentProbeViewPrimed)
			{
				...
				CaptureProbe(PrimingPixels, PrimingError, true);
				ActiveBatch->bCurrentProbeViewPrimed = true;
				ActiveBatch->CurrentProbeWarmupTicks = 0;
				ActiveBatch->CurrentProbeLastObservedFrame = GFrameCounter;
				// The initial asset gate already paid the elapsed-time floor. After a
				// rendered prime, require distinct frames and any newly visible pending
				// work without charging the initial asset gate a second time.
				ActiveBatch->CurrentProbeWarmupStartedAtSeconds =
...
void FSceneAutoTaggerController::SubmitAnalysisBatch(const TSharedPtr<FActiveBatch>& Batch)
{
	...
	FConvaiVisionService::RequestTagging(
		...
		MoveTemp(Cells),
		Params,
		[WeakSelfForCallback, CallbackRunId, CallbackBatchId](const FConvaiVisionService::FResult& Result) mutable
		{
			const TSharedPtr<FSceneAutoTaggerController> Self = WeakSelfForCallback.Pin();
			if (!Self.IsValid())
			{
				return;
			}
