...
{
	// No actions configured → don't send action_config at all (server treats absence as
	// "non-action bot"). Objects/Characters without Actions are meaningless to the LLM.
	if (Actions.Num() == 0)
	{
		return FString();
	}
