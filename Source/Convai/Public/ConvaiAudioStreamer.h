...
public:
	...
	UFUNCTION(BlueprintPure, Category = "Convai|LipSync", Meta = (Tooltip = "Returns true if the underlying lipsync system has frames it can currently produce"))
	bool HasPlayableFaceFrames() const;
...
FAudioRingBuffer LipSyncAudioRingBuffer; // For lipsync audio (only used when lipsync doesn't use precomputed data)
...
// Wall-clock time (FPlatformTime::Seconds()) when the current talking session started.
// Used as an upper bound for playback time estimation.
double AudioPlaybackStartTime;

/**
 * Returns the current audio playback time in seconds.
 * Uses byte-consumption tracking (TotalBytesQueued - AvailableBytes) and wall-clock
 * time since playback started, returning whichever is lower.
...
/**
 * Smoothed audio activity from 0 (silent) to 1, normalized to the recent voice level.
 * Use for cosmetic animation blends, or compare against e.g. 0.1 for a talking animation.
 * Pauses between words can lower this value. It does not indicate that the bot turn ended
 * and must not replace GetIsTalking for conversation, action, or interruption logic.
 */
UFUNCTION(BlueprintPure, Category = "Convai|Audio")
float GetTalkingPower() const;
...
void ProcessIncomingPrecomputedLipSync();  // Process precomputed facial animation data from server
void ProcessIncomingNonPrecomputedLipSync();  // Process audio for real-time lipsync inference
...
private:
	...
	FConvaiTalkingPower TalkingPower;
	mutable FCriticalSection TalkingPowerLock;
	double TalkingPowerLastPlaybackTime = 0.0;
	float TalkingPowerStallTime = 0.0f;
	void UpdateTalkingPower(float DeltaTime);

	// Reusable buffer for processing audio chunks (avoids repeated allocations)
	TArray<uint8> AudioChunkBuffer;

	// Reusable buffer for lipsync audio processing (avoids repeated allocations)
	TArray<uint8> LipSyncAudioChunkBuffer;

	// Per-tick dequeue cap in seconds. Small enough that a post-hitch backlog
	// dump cannot spike TotalBytesQueuedToAudio by more than this duration per
	// tick — which keeps BytesConsumed bounded under a bursty sink (e.g. Pixel
	// Streaming's WebRTC encoder). Large enough to bridge normal tick spacing
	// without underrunning SoundWaveProcedural.
	static constexpr float MaxChunkDurationSec = 0.15f; // 150 ms

	// Monotonic-clamp state for GetAudioPlaybackTime(): prevents backward jumps
	// and forward overshoot faster than wall-clock. Needed because bursty audio
	// sinks (e.g. Pixel Streaming's WebRTC encoder draining the SoundWaveProcedural
	// in a catch-up burst after a hitch) can otherwise jump the reported time
	// by hundreds of ms and cause the FaceSync frame selector to skip ahead.
	mutable double LastReportedPlaybackTime = 0.0;
	mutable double LastReportedWallClock = 0.0;
	mutable FCriticalSection PlaybackTimeLock;

protected:
	// Set from the audio thread when the stutter grace window expires with data
	// still in the ring buffer below MinBufferDuration. The game thread tick
	// reads and clears this to pass Force=true to ProcessIncomingAudio, bypassing
	// the min-buffer guard so the remainder drains and onAudioFinished fires.
	TAtomic<bool> bForceFlushPending{false};

private:

	void ResetPlaybackTimeTracking();

#if ConvaiDebugMode
	struct FConvaiTimingStat
	{
	...
	struct FConvaiAudioStreamerStats
	{
		int32 SampleCount = 0;
		FConvaiTimingStat Tick;
		FConvaiTimingStat Fade;
		FConvaiTimingStat ProcessAudio;
		FConvaiTimingStat LipSync;
		FConvaiTimingStat FinishCheck;
	...
	FConvaiAudioStreamerStats AudioStreamerStats;
#endif
