	...
	int32 AttemptNumber)
{
	...
	AsyncOp->OnComplete([this, Promise, AttemptNumber, AsyncOp](const TConvaiResult<ConvaiEditor::FHttpAsyncResponse> &Result)
						{
		if (!Promise.IsValid())
		{
		...
		if (!Result.IsSuccess())
		{
			...
			UpdateCache(ErrorResult);
			Promise->SetValue(ErrorResult);
			NotifyDelegates(ErrorResult);
			return;
		}
		...
		if (!ParseGitHubReleasesJson(HttpResponse.Body, Releases))
		{
			FScopeLock Lock(&CacheLock);
			bCheckInProgress = false;
		...
		FGitHubReleaseInfo LatestRelease = FindLatestRelease(Releases);
		...
		FUpdateCheckResult FinalResult = CreateResultFromComparison(LatestRelease);

		FScopeLock Lock(&CacheLock);
		bCheckInProgress = false;

		UpdateCache(FinalResult);
		
		AsyncTask(ENamedThreads::GameThread, [this, Promise, FinalResult]()
		{
			Promise->SetValue(FinalResult);
			NotifyDelegates(FinalResult);
		}); });
