                                ...
                                if (Tpl)
                                {
                                    ...
                                    if (Tpl->Parameters.Num() > 0)
                                    {
                                        ...
                                        for (int32 i = 0; i < FMath::Min(Tpl->Parameters.Num(), Values.Num()); ++i)
                                        {
                                            ...
                                            FConvaiResultParam Param = UConvaiActions::CoerceParam(CleanValue, Decl.Type, Env, Decl.EnumType);

                                            // Choices/Enum validation — log warning, don't drop the value.
                                            if (Decl.Type == EConvaiActionParamType::Enum)
                                            {
                                                ...
                                                // Enum membership check could go here; for now we trust the LLM
                                                // since we surfaced the choices in the wire format.
                                            }
                                            else if (Decl.Choices.Num() > 0 && !Decl.Choices.Contains(Param.StringValue))
                                            {
                                                CONVAI_LOG(ConvaiSubsystemLog, Warning,
