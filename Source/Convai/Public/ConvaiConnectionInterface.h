...
public:
    ...
    /** The character said it is ready (bot-ready) for this session: the one
     *  point at which a session - first or reconnected - can take a turn. */
    virtual void OnSessionReady() {}

    /** A connect for this owner is about to be built - a start, a retry or the
     *  microphone watchdog's restart. The owner refreshes what the payload
     *  reads, so a reconnected character is configured like a started one. */
    virtual void OnPrepareConnect() {}

    /** Reconnect is off and bot-ready never came: the session stays up without
     *  it, so whatever waits for ready goes out now, as it did before. */
    virtual void OnReadyTimedOut() {}
