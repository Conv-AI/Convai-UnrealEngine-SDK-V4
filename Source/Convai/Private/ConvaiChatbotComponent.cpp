...
{
	IConvaiLipSyncInterface* LipSyncComp = FindFirstLipSyncComponent();

	if (!LipSyncComp)
		return EC_LipSyncMode::Off;

	if (!LipSyncComp->RequiresPrecomputedFaceData())
	{
		return EC_LipSyncMode::Off;
	}
	
	if (LipSyncComp->GeneratesFacialDataAsBlendshapes())
	{
...
{
	UE_LOG(LogTemp, Display, TEXT("Audio left %f"), AudioRingBuffer.GetTotalDuration());
	StopVoice();
