...
namespace
{
	// Two seconds at 48 kHz stereo. Only reachable if the drain stops running
	// entirely; shedding beats growing without bound and then flushing seconds
	// of stale audio into the session at once.
	constexpr int32 kMicTapMaxSamples = 2 * 2 * 48000;
}
                                                 ...
                                                 int32 NumChannels, int32 SampleRate)
{
	...
	if (MicTapBuffer.Num() + NumSamples > kMicTapMaxSamples)
	{
		const int32 Overflow = MicTapBuffer.Num() + NumSamples - kMicTapMaxSamples;
		...
		if (!bWarnedMicTapOverflow)
		{
			...
			CONVAI_LOG(ConvaiPlayerLog, Warning,
				TEXT("Microphone tap buffer overflowed; the drain is not keeping up and audio is being dropped."));
		}
	}
