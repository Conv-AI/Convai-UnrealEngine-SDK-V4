...
public:
	...
	*/
	 ...
	 *
	 * bEphemeral: when true, the event is shown to the AI EXACTLY ONCE (in the next context
	 * update) and is never persisted — it won't reappear on later updates, as if it was never
	 * added. Use for transient cues ("the player just glanced at the door") that should nudge
	 * the AI once without piling up. When false (default) the event persists in the running context.
	 */
	UFUNCTION(BlueprintCallable, Category = "Convai|DynamicContext",
		meta = (DisplayName = "Add Context Event", AdvancedDisplay = "bEphemeral,bFlushImmediately"))
	 ...
	 * @param bFlushImmediately  Send right away instead of batching. Use sparingly
	 *                  — high-frequency calls will spam the connection.
	 */
	UFUNCTION(BlueprintCallable, Category = "Convai|DynamicContext",
		meta = (DisplayName = "Set Context Fact", AdvancedDisplay = "Key,bFlushImmediately"))
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Convai|Actions",
		meta = (AdvancedDisplay = "bAddAttentionEvent,bFlushImmediately",
				ToolTip = "Set attention object. By default also emits a one-flush event '<player> is paying attention to <object>' (optional Text appended). Has no effect when Environment > Enable Actions is off."))
