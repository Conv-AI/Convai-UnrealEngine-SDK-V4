...
void UConvaiChatbotComponent::AddActionHandler(UObject* Handler)
{
	if (IsValid(Handler))
	{
		ActionHandlers.AddUnique(Handler);
	}
}

void UConvaiChatbotComponent::RemoveActionHandler(UObject* Handler)
{
	ActionHandlers.Remove(TWeakObjectPtr<UObject>(Handler));
}
...
bool UConvaiChatbotComponent::TriggerNamedBlueprintAction(const FString& ActionName, FConvaiResultAction ConvaiActionStruct)
{
	if (!ActionName.IsEmpty())
	{
		...
		// Registered handlers — the avatar's behaviour component. After the owner so a class
		// that already carries the action keeps winning, before self so a handler still beats
		// a built-in.
		for (const TWeakObjectPtr<UObject>& HandlerPtr : ActionHandlers)
		{
			...
			ActiveActionHandlerTarget = Handler;
			if (TryCallFunction(Handler, ActionName, ConvaiActionStruct))
			{
				return true;
			}
			ActiveActionHandlerTarget.Reset();
...
void UConvaiChatbotComponent::InvokeNarrativeDesignTrigger(const FString TriggerName)
{
	...
	if (!IsChatbotConnected())
	{
		...
		Pending.QueuedAtSeconds = FPlatformTime::Seconds();
...
void UConvaiChatbotComponent::BeginPlay()
{
	...
	if (UWorld* World = GetWorld();
		World && ShouldAutoInitializeSession(bAutoInitializeSession, World->GetNetMode()))
	{
		{
...
void UConvaiChatbotComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	// Handlers are set by whoever spawned the avatar and outlive this component;
	// drop the references so a torn-down chatbot cannot keep them alive.
...
void UConvaiChatbotComponent::OnConvaiGetDetailsCompleted(FString ReceivedCharacterName, FString ReceivedVoiceType, FString ReceivedBackstory, FString ReceivedLanguageCode, bool HasReadyPlayerMeLink, FString ReceivedReadyPlayerMeLink, FString ReceivedAvatarImageLink)
{
	if (ReceivedCharacterName == "" && ReceivedVoiceType == "" && ReceivedBackstory == "")
	{
		...
		// A character the service refuses is a session that will never work, and
		// until now the only routes that said so were this log line and a bool on
		// OnCharacterDataLoaded. A game bound to OnFailureEvent -- the documented
		// place to show an error -- was told nothing, and one polling for
		// Connected waited forever on a component that never left Disconnected
		// (FINDINGS F38).
...
void UConvaiChatbotComponent::OnDisconnectedFromServer()
{
	...
	AsyncTask(ENamedThreads::GameThread, [WeakSelf, ExpectedEpoch]
	{
		if (UConvaiChatbotComponent* Self = WeakSelf.Get();
			Self && Self->ActionLifecycleEpoch.Load() == ExpectedEpoch)
		{
			Self->NoteDropForReplay();
			Self->ResetActionExecution(/*bLatchShutdown*/ false);
		}
	});
}

void UConvaiChatbotComponent::NoteDropForReplay()
{
	// The first drop of a chain: the gap is measured from when the character
	// went away, not from its last failed retry.
	SessionBackAtSeconds = -1.0;
	if (DropAtSeconds < 0.0)
	{
		DropAtSeconds = FPlatformTime::Seconds();
		const UConvaiSubsystem* ConvaiSubsystem = UConvaiUtils::GetConvaiSubsystem(this);
		DropReason = ConvaiSubsystem ? ConvaiSubsystem->GetLastDisconnectReason() : EC_DisconnectReason::Unexpected;
	}
	if (ActionExecutionState == EConvaiActionExecutionState::Running && ActionsQueue.Num() > 0)
	{
		TArray<FConvaiAction> ActionTemplates;
		GetActionsForParsing(ActionTemplates);
		InterruptedActionLabel = BuildManagedActionLabel(ActionsQueue[0], ActionTemplates);
	}
	// A face frozen mid-emotion reads as the character still reacting to a
	// line the player never finished hearing.
	if (!LockEmotionState)
	{
		ResetEmotionState();
	}
...
void UConvaiChatbotComponent::ReplayStagedWorkAfterGap()
{
	const double Now = FPlatformTime::Seconds();
	const double Gap = DropAtSeconds < 0.0 ? 0.0 : Now - DropAtSeconds;
	const bool bReplay = DropAtSeconds < 0.0 || ShouldReplayAfterGap(Gap, DropReason);

	const int32 Before = PendingTriggers.Num();
	const double BackAt = SessionBackAtSeconds;
	PendingTriggers.RemoveAll([bReplay, Now, BackAt](const FPendingTrigger& Trigger)
	{
		if (BackAt >= 0.0 && Trigger.QueuedAtSeconds >= BackAt)
		{
			return false;
		}
		return !bReplay || Now - Trigger.QueuedAtSeconds >= kReplayMaxGapSeconds;
	});
	if (PendingTriggers.Num() != Before)
	{
		CONVAI_LOG(ConvaiChatbotComponentLog, Log, TEXT("Dropped %d narrative trigger(s) staged before a %.0f s gap"),
			Before - PendingTriggers.Num(), Gap);
	}

	// Kept as facts, but nothing staged for a moment that has passed may make
	// the character speak now.
	if (!bReplay)
	{
		PendingContextBatch.DowngradeToNever();
		HeldContext.DowngradeToNever();
	}

	if (!InterruptedActionLabel.IsEmpty())
	{
		AddContextEvent(FString::Printf(TEXT("Action %s was stopped by a connection loss."), *InterruptedActionLabel),
			EC_RunLLMOption::Never);
	}

	DropAtSeconds = -1.0;
	InterruptedActionLabel.Reset();
}
...
void UConvaiChatbotComponent::BroadcastConnectionStateChanged(const FString& AttendeeId, EC_ConnectionState State)
{
	if (!IsInGameThread())
	{
		...
		AsyncTask(ENamedThreads::GameThread, [WeakThis, AttendeeId, State]()
		{
			...
			{
				Component->BroadcastConnectionStateChanged(AttendeeId, State);
			}
		});
		return;
	}

	// Edge-triggered on Disconnected: one departure is one Disconnected.
	// Several paths report the same drop — the transport's attendee loop, the
	// lease release, the component's own teardown — and a listener that
	// restarts the session on Disconnected restarted once per path. Only the
	// repeat is dropped: a retry legitimately reports Connecting per attempt,
	// so nothing else is deduplicated.
	if (State == EC_ConnectionState::Disconnected &&
		LastBroadcastConnectionState == EC_ConnectionState::Disconnected)
	{
		return;
	}
	LastBroadcastConnectionState = State;

	OnAttendeeConnectionStateChangedEvent.Broadcast(this, AttendeeId, State);
}

void UConvaiChatbotComponent::OnSessionReady()
{
	CONVAI_LOG(ConvaiChatbotComponentLog, Log, TEXT("Session ready for character ID: %s"), *CharacterID);
	ApplySessionConfig();
}

void UConvaiChatbotComponent::OnReadyTimedOut()
{
	ApplySessionConfig();
}

void UConvaiChatbotComponent::ApplySessionConfig()
{
	ReplayStagedWorkAfterGap();
	bSessionReady = true;

	// Every server session starts empty, a reconnected one included. Sent at
	// ready rather than at connect: before it the bot is not listening yet.
	if (IsValid(SessionProxyInstance))
	{
		SessionProxyInstance->UpdateTemplateKeys(NarrativeTemplateKeys);
		SessionProxyInstance->UpdateDynamicInfo(DynamicEnvironmentInfo);
	}
	// The derived Instructions block joins a silent canonical Replace and
	// supersedes the legacy stable-fact hints from older builds.
	SyncManagedActionContext();
}

void UConvaiChatbotComponent::OnPrepareConnect()
{
	// Start Session already did this for the connect it is making.
	if (bPreparedForNextConnect)
	{
		bPreparedForNextConnect = false;
		return;
	}
	PrepareSessionEnvironment();
}

void UConvaiChatbotComponent::PrepareSessionEnvironment()
{
	...
	// What changed while the character was away, so a reconnect that sees a
	// different world says so.
	TSet<FString> ObjectNames;
	for (const FConvaiObjectEntry& Object : EnvironmentData.Objects)
	{
		ObjectNames.Add(Object.Name);
	}
	if (!ObjectNamesAtLastPrepare.IsEmpty() || !ObjectNames.IsEmpty())
	{
		const TArray<FString> Added = ObjectNames.Difference(ObjectNamesAtLastPrepare).Array();
		const TArray<FString> Removed = ObjectNamesAtLastPrepare.Difference(ObjectNames).Array();
		if (Added.Num() > 0 || Removed.Num() > 0)
		{
			CONVAI_LOG(ConvaiChatbotComponentLog, Log, TEXT("Environment for [%s]: objects added [%s] removed [%s]"),
				*CharacterID, *FString::Join(Added, TEXT(", ")), *FString::Join(Removed, TEXT(", ")));
		}
	}
	ObjectNamesAtLastPrepare = MoveTemp(ObjectNames);

	// Identity is fixed per session. A different player is a different Visit,
	// not the same one coming back.
	const FString CurrentEndUserID = GetEndUserID();
	if (!EndUserIDAtLastPrepare.IsEmpty() && EndUserIDAtLastPrepare != CurrentEndUserID)
	{
		CONVAI_LOG(ConvaiChatbotComponentLog, Log, TEXT("End User ID for [%s] changed; starting a new Visit"), *CharacterID);
		if (UConvaiSubsystem* ConvaiSubsystem = UConvaiUtils::GetConvaiSubsystem(this))
		{
			ConvaiSubsystem->BeginVisit();
		}
	}
	EndUserIDAtLastPrepare = CurrentEndUserID;
}

bool UConvaiChatbotComponent::IsSessionReady() const
{
	const UConvaiSubsystem* ConvaiSubsystem = UConvaiUtils::GetConvaiSubsystem(this);
	return ConvaiSubsystem && ConvaiSubsystem->IsSessionReady(SessionProxyInstance);
}
...
void UConvaiChatbotComponent::OnAttendeeConnected(FString AttendeeId)
{
	...
	SessionBackAtSeconds = FPlatformTime::Seconds();
...
void UConvaiChatbotComponent::OnEmotionReceived(FString ReceivedEmotionResponse, FAnimationFrame EmotionBlendshapesFrame, bool MultipleEmotions)
{
	...
	if (!ReceivedEmotionResponse.IsEmpty())
	{
		...
		float DominantScore = 0.f;
		for (int32 i = 0; i <= static_cast<int32>(EBasicEmotions::Anticipation); ++i)
		{
			if (const float Score = EmotionState.GetEmotionScore(static_cast<EBasicEmotions>(i)); Score > DominantScore)
			{
				Dominant = static_cast<EBasicEmotions>(i);
				DominantScore = Score;
			}
		}
		CONVAI_LOG(ConvaiChatbotComponentLog, Log, TEXT("Emotion \"%s\" -> %s %.2f"), *ReceivedEmotionResponse,
			*StaticEnum<EBasicEmotions>()->GetNameStringByValue(static_cast<int64>(Dominant)), DominantScore);
	}
...
void UConvaiChatbotComponent::OnServerError(const FString& Message, bool bFatal)
{
	TWeakObjectPtr<UConvaiChatbotComponent> WeakSelf(this);
	const FString MessageCopy = Message;
	AsyncTask(ENamedThreads::GameThread, [WeakSelf, MessageCopy, bFatal] {
		if (WeakSelf.IsValid())
		{
			WeakSelf->OnServerErrorEvent.Broadcast(WeakSelf.Get(), MessageCopy, bFatal);
		}
	});

	// OnFailureEvent keeps its meaning: fatal only. A game already bound to it sees
	// no change, which is the point -- one measured session received 17 non-fatal
	// advisories, and routing those here produced 19 failure events for a connection
	// that never broke.
	if (bFatal)
	{
		OnFailure(Message);
...
bool UConvaiChatbotComponent::ShouldAutoInitializeSession(bool bFlag, ENetMode NetMode)
{
	// A dedicated server has no player to talk to and no microphone to listen
	// with, so a character that auto-connected there opened a session, spent a
	// concurrency slot and held it for the life of the level.
	return bFlag && NetMode != NM_DedicatedServer;
}

void UConvaiChatbotComponent::StartSession()
{
	StartSessionInternal(/*bFromDisconnectBroadcast*/ false);
}

void UConvaiChatbotComponent::StartSessionInternal(bool bFromDisconnectBroadcast)
{
	if (UConvaiSubsystem* ConvaiSubsystem = UConvaiUtils::GetConvaiSubsystem(this))
	{
		// The forum's wiring - Start Session on Disconnected - after an idle close
		// would open a session for an empty room. It is kept as intent instead: the
		// player's next word or keystroke brings the character back.
		if (ConvaiSubsystem->ShouldHoldStartForWake(SessionProxyInstance))
		{
			return;
		}

		// From inside a disconnect event, next tick: the drop that is being
		// announced has not decided on its retry yet, and this call is about
		// to be adopted into it.
		if (ConvaiSubsystem->IsInsideDisconnectBroadcast())
		{
			TWeakObjectPtr<UConvaiChatbotComponent> WeakThis(this);
			const uint32 Requests = SessionRequestSerial;
			AsyncTask(ENamedThreads::GameThread, [WeakThis, Requests]()
			{
				// A Stop Session after this start, in the same broadcast, is
				// the game's last word.
				if (UConvaiChatbotComponent* Chatbot = WeakThis.Get(); Chatbot && Chatbot->SessionRequestSerial == Requests)
				{
					Chatbot->StartSessionInternal(/*bFromDisconnectBroadcast*/ true);
				}
			});
			return;
		}

		// The legacy restart of a session whose retry is pending: that retry,
		// fired now, is the restart - same Visit, same chain.
		if (ConvaiSubsystem->AdoptStartSession(SessionProxyInstance, CharacterID))
		{
			return;
		}

		// A game re-running its own wiring on a disconnect is not choosing
		// another character.
		if (!bFromDisconnectBroadcast)
		{
			ConvaiSubsystem->SupersedeReconnect(CharacterID);
		}
	}
	...
	// A session the game starts is a new Visit: nothing staged against the
	// old one is measured against a gap.
	bSessionReady = false;
	DropAtSeconds = -1.0;
	InterruptedActionLabel.Reset();
	...
	// A new session gets a fresh edge: the Disconnected filter below is there
	// to collapse ONE departure reported by several paths, not to swallow the
	// end of the next session.
	LastBroadcastConnectionState = EC_ConnectionState::Connecting;

	// A start the game asked for is what begins a Visit: the AFK clock, the
	// talking reprieve and the disconnect verdict all belong to the player's
	// presence, not to one connection. A reconnect deliberately does not come
	// through here.
	if (UConvaiSubsystem* ConvaiSubsystem = UConvaiUtils::GetConvaiSubsystem(this))
	{
		ConvaiSubsystem->BeginVisit();
	...
	// Here rather than only in OnPrepareConnect: a warm rebind reuses a live
	// session and never builds a connect, and still takes this character's world.
	PrepareSessionEnvironment();
	bPreparedForNextConnect = true;
	{
		UConvaiSubsystem* ConvaiSubsystem = UConvaiUtils::GetConvaiSubsystem(this);
		TOptional<TGuardValue<bool>> DeferredScope;
		if (ConvaiSubsystem && bFromDisconnectBroadcast)
		{
			DeferredScope.Emplace(ConvaiSubsystem->bStartDeferredFromDisconnect, true);
		}
...
void UConvaiChatbotComponent::StopSession()
{
	...
	if (IsValid(SessionProxyInstance))
	{
		// The verdict is not set here any more. Returning the proxy may leave
		// the transport up for the orphan grace, and whoever actually tears it
		// down marks that epoch Explicit — including the orphan's own expiry.

		// Null the member BEFORE returning the proxy. Returning it notifies this
		// component's own handlers, and a game that restarts the session from
		// there must find a component with no session rather than one still
		// holding the proxy being returned.
		UConvaiConnectionSessionProxy* ReturningProxy = SessionProxyInstance;
		SessionProxyInstance = nullptr;

		// Before the return: a retry the plugin had scheduled for this session
		// would otherwise reopen it after the game stopped it.
		if (UConvaiSubsystem* ConvaiSubsystem = UConvaiUtils::GetConvaiSubsystem(this))
		{
			ConvaiSubsystem->StopReconnect(ReturningProxy);
		}
		UConvaiConnectionSessionProxy::ReturnSessionProxy(this, CharacterID, ReturningProxy);
