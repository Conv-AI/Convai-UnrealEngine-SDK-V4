...
{
// Keep this workspace on the same shared palette and button states as other Convai tools.
using ConvaiEditorVisual::FromHex;
using ConvaiEditorVisual::Window;
using ConvaiEditorVisual::Header;
using ConvaiEditorVisual::Toolbar;
using ConvaiEditorVisual::Panel;
using ConvaiEditorVisual::Row;
using ConvaiEditorVisual::RowHover;
using ConvaiEditorVisual::SelectedRow;
using ConvaiEditorVisual::Preview;
using ConvaiEditorVisual::Border;
using ConvaiEditorVisual::PrimaryText;
using ConvaiEditorVisual::SecondaryText;
using ConvaiEditorVisual::MutedText;
using ConvaiEditorVisual::Accent;
using ConvaiEditorVisual::PrimaryAction;
using ConvaiEditorVisual::PrimaryActionHover;
using ConvaiEditorVisual::PrimaryActionPressed;
using ConvaiEditorVisual::DisabledAction;
using ConvaiEditorVisual::DisabledText;
using ConvaiEditorVisual::PrimaryButtonStyle;
using ConvaiEditorVisual::SecondaryButtonStyle;
	...
	[
		...
		[
			...
			[
				...
				+ SOverlay::Slot()
				[
					SNew(SConvaiAvatarPortrait)
					.Image_Lambda([this, AvatarUrl]() { return GetCharacterAvatarBrush(AvatarUrl); })
					.IsLoading_Lambda([this, AvatarUrl]() { return !AvatarUrl.IsEmpty() && PendingCharacterAvatarDownloads.Contains(AvatarUrl); })
					.FallbackText(FText::FromString(Initial))
					.FallbackFont(SceneAutoTaggerVisual::CharacterNameFont(24))
					// Preserve the established full-width, bottom-anchored character framing.
					.ImageStretch(EStretch::ScaleToFitX)
				]
...
TSharedRef<SWidget> SConvaiSceneAutoTagger::BuildBatchActionsMenu()
{
	...
	const TWeakPtr<SConvaiSceneAutoTagger> WeakWidget = SharedThis(this);
	...
	MenuBuilder.AddMenuEntry(
		...
		FSlateIcon(),
		FUIAction(
			FExecuteAction::CreateLambda([WeakWidget]()
			{
				if (const TSharedPtr<SConvaiSceneAutoTagger> Self = WeakWidget.Pin())
				{
					Self->HandleBatchDecision(ESceneAutoTaggerDecision::Accepted);
				}
			}),
			FCanExecuteAction::CreateLambda([WeakWidget]()
			{
				const TSharedPtr<SConvaiSceneAutoTagger> Self = WeakWidget.Pin();
	...
	if (!RefinementNoteActorPaths.IsEmpty())
	{
		MenuBuilder.AddMenuEntry(
			...
			FUIAction(
				FExecuteAction::CreateLambda([WeakWidget, RefinementNoteActorPaths]()
				{
					if (const TSharedPtr<SConvaiSceneAutoTagger> Self = WeakWidget.Pin())
					{
				...
				FCanExecuteAction::CreateLambda([WeakWidget, RefinementNoteActorPaths]()
				{
					const TSharedPtr<SConvaiSceneAutoTagger> Self = WeakWidget.Pin();
	...
	MenuBuilder.AddMenuEntry(
		...
		FSlateIcon(),
		FUIAction(FExecuteAction::CreateLambda([WeakWidget]()
		{
			if (const TSharedPtr<SConvaiSceneAutoTagger> Self = WeakWidget.Pin())
			{
...
TSharedPtr<SWidget> SConvaiSceneAutoTagger::BuildCandidateContextMenu()
{
	...
	if (!ActorPaths.IsEmpty())
	{
		...
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateLambda([WeakWidget, ActorPaths]()
				{
					if (const TSharedPtr<SConvaiSceneAutoTagger> Self = WeakWidget.Pin())
					{
				...
				FCanExecuteAction::CreateLambda([WeakWidget, ActorPaths]()
				{
					const TSharedPtr<SConvaiSceneAutoTagger> Self = WeakWidget.Pin();
					return Self.IsValid() && !Self->bEditingView && Self->Controller.IsValid()
						&& !Self->Controller->IsBusy()
...
void SConvaiSceneAutoTagger::WaitForPreviewResource(const uint64 ResourceGeneration)
{
	...
	FTSTicker::GetCoreTicker().AddTicker(
		FTickerDelegate::CreateLambda(
			[WeakWidget, ResourceFence, ResourceGeneration](float)
			{
				const TSharedPtr<SConvaiSceneAutoTagger> Pinned = WeakWidget.Pin();
				if (!Pinned.IsValid())
				{
					return false;
				}
...
void SConvaiSceneAutoTagger::RefreshCharacterCatalog()
{
	...
	RefreshCharacterGrid();
	const TWeakPtr<SConvaiSceneAutoTagger> WeakWidget = SharedThis(this);
	CharacterCatalog->Refresh([WeakWidget](bool bSuccess, TArray<FSceneAutoTaggerCharacterOption> Characters, FString Error)
	{
		const TSharedPtr<SConvaiSceneAutoTagger> Pinned = WeakWidget.Pin();
...
void SConvaiSceneAutoTagger::SubmitSceneContextDraftRequest(const uint64 RequestGeneration)
{
	...
	FConvaiVisionService::RequestSceneContextDescription(
		...
		Params,
		[WeakWidget, RequestGeneration, CharacterID](const FConvaiVisionService::FResult& Result)
		{
			const TSharedPtr<SConvaiSceneAutoTagger> Pinned = WeakWidget.Pin();
