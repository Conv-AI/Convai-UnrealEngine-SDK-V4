...
/** Cached command-line / settings overrides for the anim node.
 *  Resolved once per Initialize_AnyThread (i.e. once per PIE session). */
struct FLipSyncAnimOverrides
{
	TOptional<float> UpperFaceAlpha;
	TOptional<float> LowerFaceAlpha;
	TOptional<float> StarvationBlendInDuration;
	TOptional<float> StarvationBlendOutDuration;
	TOptional<float> GlobalMultiplier;
	TOptional<float> GlobalOffset;
	TOptional<float> LowerSmoothingSpeed;
	TOptional<float> UpperSmoothingSpeed;
	TOptional<bool>  bEnableLowerSmoothing;
	TOptional<bool>  bEnableUpperSmoothing;
	TOptional<bool>  bBypassSmoothing;
	TOptional<bool>  bBypassStarvationBlend;
	TOptional<EConvaiFaceSyncApplyMode> ApplyMode;
};
...
struct CONVAI_API FAnimNode_ConvaiFaceSync : public FAnimNode_Base
{
	...
	// ── Smoothing ──
	// All values below can be overridden at launch via -LipSyncAnim* command-line flags.
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Convai|Smoothing", meta = (PinHiddenByDefault, ClampMin = "0.0", ClampMax = "1.0", EditCondition = "bEnableLowerFaceSmoothing"))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Convai|Smoothing", meta = (PinHiddenByDefault, ClampMin = "0.0", ClampMax = "1.0", EditCondition = "bEnableUpperFaceSmoothing"))
	float UpperFaceSmoothingSpeed = 1.0f;

	// ── Starvation Blend ──

	/** Duration (seconds) to blend face curves IN when lipsync frames become available.
	 *  Applied as a constant rate — a full 0→1 transition takes this amount of time.
	 *  A mid-transition reversal takes proportionally less. Set to 0 for an instant switch. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Convai|Starvation", meta = (PinHiddenByDefault, ClampMin = "0.0"))
	float StarvationBlendInDuration = 0.1f;

	/** Duration (seconds) to blend face curves OUT when the lipsync system runs out of frames.
	 *  Applied as a constant rate — a full 1→0 transition takes this amount of time.
	 *  A mid-transition reversal takes proportionally less. Set to 0 for an instant switch. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Convai|Starvation", meta = (PinHiddenByDefault, ClampMin = "0.0"))
...
private:
	...
	/** Current starvation blend alpha — 1 = fully applying curves, 0 = fully faded out */
	float CurrentStarvationAlpha = 0.0f;

	/** Resolved component — either from the pin or auto-discovered */
	TWeakObjectPtr<UConvaiChatbotComponent> ResolvedComponent;

	/** Cached param overrides — resolved once in Initialize_AnyThread per PIE session */
	FLipSyncAnimOverrides CachedOverrides;
