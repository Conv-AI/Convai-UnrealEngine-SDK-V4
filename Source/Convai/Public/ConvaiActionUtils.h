...
public:
	 ...
	 *                            is already at the goal: horizontal distance
	 *                            from Source Actor to Out Goal Location is
	 *                            within max(Acceptance Radius × 2, 150 uu)
	 *                            AND |Z delta to the nearest face of the
	 *                            entry's bounding box| is within Source
	 *                            Actor's own height. Branch on this before
	 *                            calling AI Move To to short-circuit a no-op
	 *                            move when the actor is already arrived.
	 *   - bOut Reachable:        true when Source Actor was provided AND the
	 *                            navmesh path landed within ~300 uu
	 *                            (horizontal) + Source Actor's own vertical
	 *                            span of the entry's bounds. False when no
	 *                            Source Actor, no nav system, no nav data,
	 *                            path failure, or out of tolerance.
