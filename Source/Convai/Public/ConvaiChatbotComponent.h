...
DECLARE_DYNAMIC_MULTICAST_SPARSE_DELEGATE_ThreeParams(FOnServerErrorSignature, UConvaiChatbotComponent, OnServerErrorEvent, UConvaiChatbotComponent*, ChatbotComponent, FString, Message, bool, bFatal);
...
public:
	...
	*/
	 ...
	 */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Convai|Emotion")
	float EmotionOffset = 0.2f;
	...
	/**
	 *    Registers an object searched for a Blueprint event matching the action name, after
	 *    the owner actor and before this component. The owner still wins, so an avatar class
	 *    that already handles an action keeps handling it. Null and duplicates are ignored.
	 */
	UFUNCTION(BlueprintCallable, Category = "Convai|Actions")
	void AddActionHandler(UObject* Handler);

	UFUNCTION(BlueprintCallable, Category = "Convai|Actions")
	void RemoveActionHandler(UObject* Handler);

	const TArray<TWeakObjectPtr<UObject>>& GetActionHandlers() const { return ActionHandlers; }
...
public:
	...
	/**
	 * What the server said, and whether it ended the session. OnFailureEvent carries
	 * no message and fires only on a fatal error; this carries the text for both
	 * severities. Non-fatal advisories arrive here and nowhere else -- before this
	 * existed they were logged and dropped (FINDINGS F9).
	 */
	UPROPERTY(BlueprintAssignable, Category = "Convai", meta = (DisplayName = "On Server Error"))
	FOnServerErrorSignature OnServerErrorEvent;

	/** Whether this character can answer now: the server's bot said it is
	 *  ready for this session. Connected alone is only the transport. */
...
private:
	...
	/** Staged work older than this is not replayed after a drop. */
	static constexpr double kReplayMaxGapSeconds = 30.0;
	/** Whether what was staged while the session was down still makes sense:
	 *  a short network drop, not a close the server or the game chose. */
	static bool ShouldReplayAfterGap(double GapSeconds, EC_DisconnectReason Reason)
	{
		return GapSeconds < kReplayMaxGapSeconds && Reason == EC_DisconnectReason::Unexpected;
	}
	/** When this chain's first drop happened, or -1. */
	double DropAtSeconds = -1.0;
	EC_DisconnectReason DropReason = EC_DisconnectReason::Unexpected;
	/** The action a drop cut off, told to the character once it is back. */
	FString InterruptedActionLabel;
	/** Bot-ready reached for this session: context and triggers go out only
	 *  to a character that can take them. */
	bool bSessionReady = false;
	/** When this session's character joined, or -1: triggers queued after it
	 *  belong to this session, not to the outage before it. */
	double SessionBackAtSeconds = -1.0;
	/** Counts Stop Session calls, so a start deferred out of a disconnect
	 *  broadcast can tell it was stopped again before it ran. */
	uint32 SessionRequestSerial = 0;
	void NoteDropForReplay();
	void ReplayStagedWorkAfterGap();
...
public:
	/** This character's Connection, or null before its session starts. */
	UConvaiConnectionSessionProxy* GetSessionProxy() const { return SessionProxyInstance; }
...
private:
	void StartSessionInternal(bool bFromDisconnectBroadcast);
	...
	virtual void OnSessionReady() override;
	virtual void OnPrepareConnect() override;
	virtual void OnReadyTimedOut() override;
	...
	virtual void OnServerError(const FString& Message, bool bFatal) override;
...
public:
	/** Whether this component opens a session by itself. Pure so the rule can
	 *  be tested without a world; see the dedicated-server case inside. */
	static bool ShouldAutoInitializeSession(bool bFlag, ENetMode NetMode);
...
private:
	...
	/** The last state actually broadcast, so one departure reported by several
	 *  paths is one Disconnected to a listener. */
	EC_ConnectionState LastBroadcastConnectionState = EC_ConnectionState::Disconnected;
...
private:
	...
	/** Set through AddActionHandler; cleared in EndPlay. Not a UPROPERTY on purpose: cooked
	 *  avatar paks store this class unversioned, and a reflected property added here would
	 *  shift every later property index in every one of them. */
	TArray<TWeakObjectPtr<UObject>> ActionHandlers;
	...
	/** Partner, Blueprint extras and world objects, read into the connect
	 *  payload. Run before every connect, not only the game's Start. */
	void PrepareSessionEnvironment();
	/** Keys, dynamic info and the action context: what a new server session
	 *  needs once the character can take them. */
	void ApplySessionConfig();
	/** Start Session prepared the connect it is about to make. */
	bool bPreparedForNextConnect = false;
	TSet<FString> ObjectNamesAtLastPrepare;
	FString EndUserIDAtLastPrepare;

#if WITH_DEV_AUTOMATION_TESTS
	friend struct FConvaiChatbotReconnectTestAccessor;
	friend struct FConvaiActionPlanTestAccessor;
	friend struct FConvaiActionHandlerTestAccessor;
#endif
