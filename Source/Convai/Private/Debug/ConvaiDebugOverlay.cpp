...
void SConvaiDebugOverlay::RefreshData()
{
	...
	if (Bot)
	{
		...
		if (ConvaiSub)
		{
			// The coordinator is Backoff or Connecting only inside a chain.
			const EConvaiReconnectState State = ConvaiSub->GetReconnectCoordinator().State();
			const FConvaiReconnectStatus& Status = ConvaiSub->GetLastReconnectStatus();
			if (State == EConvaiReconnectState::Dormant)
			{
				Header += TEXT("  · DORMANT");
			}
			else if (State == EConvaiReconnectState::Backoff || State == EConvaiReconnectState::Connecting)
			{
				Header += FString::Printf(TEXT("  · RECONNECTING %d/%d"), Status.Attempt, Status.MaxAttempts);
			}
		}
	}
		...
		for (const UConvaiContextSubsystem::FConvaiDebugSpatialFact& F : Facts)
		{
			...
			if (!Existing)
			{
				...
				[
					...
					// Verbatim spatial sentence (closest / in-front / distances) —
					// always shown regardless of ConvaiDebugMode.
					+ SVerticalBox::Slot().AutoHeight()
					[
						SAssignNew(NewRow.Text, STextBlock)
