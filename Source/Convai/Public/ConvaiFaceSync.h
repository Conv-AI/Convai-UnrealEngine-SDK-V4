...
public:
	...
	virtual TMap<FName, float> ConvaiGetFaceBlendshapes() const override { FScopeLock Lock(&BlendShapesMapLock); return CurrentBlendShapesMap; }
	virtual void ForceRecalculateStartTime() override;
	virtual bool HasPlayableFrames() const override;
	...
	virtual void SetCurrentFrameToZero() { FScopeLock Lock(&BlendShapesMapLock); CurrentBlendShapesMap = GetZeroFrameForMode(GetLipSyncMode()); }
	TMap<FName, float> GetCurrentFrame() { FScopeLock Lock(&BlendShapesMapLock); return CurrentBlendShapesMap; }
...
protected:
	...
	bool bIsPlaying;
	bool bIsPaused;
	/** Temporary data starvation is not an intentional playback pause. Tick must
	 *  continue so newly arrived frames can recover against the audio clock. */
	bool bWaitingForFaceFrames = false;
	bool bStarvationFallbackApplied = false;
	bool bSimulateFreeze = false;

	//~ Thread Synchronization
	FCriticalSection SequenceCriticalSection;
	FCriticalSection RecordingCriticalSection;
	// Guards every read/write of CurrentBlendShapesMap. The game thread mutates it in
	// TickComponent while the animation worker thread copies it via ConvaiGetFaceBlendshapes()
	// in FAnimNode_ConvaiFaceSync::Evaluate_AnyThread(). mutable so the const getter can lock.
	mutable FCriticalSection BlendShapesMapLock;
	...
	struct FConvaiFaceSyncStats
	{
		int32 SampleCount = 0;
		FConvaiTimingStat Tick;
		FConvaiTimingStat FrameSel;
		FConvaiTimingStat Interp;
		FConvaiTimingStat PostProc;
		FConvaiTimingStat Delegate;
	...
	FConvaiFaceSyncStats FaceSyncStats;
#endif
};
