...
private:
	...
	struct FObserverSpatialCache
	{
		...
		struct FFact
		{
			FString Name;
			FString Sentence;
			// Sentence minus the player-perspective clause. When only that clause
			// changed (the player moved/turned), the fact republishes silently.
			FString EgoSentence;
		};
		TMap<FString, FFact> Facts;

		// True when the last pass appended a player-perspective clause to at
		// least one fact. Gates the frame-selection prompt instruction.
		bool bHasPlayerPerspectiveFacts = false;
...
public:
	...
	/** True when Observer's published spatial facts currently carry a
	 *  player-perspective ("From <name>'s position") clause — i.e. the feature is
	 *  enabled, surroundings are received, and a player exists. Gates the
	 *  frame-selection prompt instruction so it never advertises a clause that
	 *  isn't in the context. */
	bool HasPlayerPerspectiveFacts(const class UConvaiChatbotComponent* Observer) const;
};
