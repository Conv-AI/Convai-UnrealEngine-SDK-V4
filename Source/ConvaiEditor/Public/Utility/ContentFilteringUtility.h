...
struct FSemanticVersion
{
    ...
    int32 Minor;
    int32 Patch;
    FString PreReleaseLabel;
    int32 PreReleaseNumber;

    /** Default constructor (invalid version) */
    FSemanticVersion()
        : Major(-1), Minor(-1), Patch(-1), PreReleaseLabel(TEXT("")), PreReleaseNumber(-1)
    {
    }

    /** Explicit constructor */
    FSemanticVersion(int32 InMajor, int32 InMinor = 0, int32 InPatch = 0)
        : Major(InMajor), Minor(InMinor), Patch(InPatch), PreReleaseLabel(TEXT("")), PreReleaseNumber(-1)
    {
    }
    ...
    static FSemanticVersion Parse(const FString &VersionString)
    {
        ...
        FString NormalizedVersion = VersionString;
        if (NormalizedVersion.StartsWith(TEXT("v")) || NormalizedVersion.StartsWith(TEXT("V")))
        {
            NormalizedVersion = NormalizedVersion.RightChop(1);
        }

        FString CoreVersion = NormalizedVersion;
        FString PreRelease;
        if (NormalizedVersion.Split(TEXT("-"), &CoreVersion, &PreRelease))
        {
            CoreVersion = CoreVersion.TrimStartAndEnd();
            PreRelease = PreRelease.TrimStartAndEnd();
        }

        TArray<FString> Parts;
        CoreVersion.ParseIntoArray(Parts, TEXT("."));
        ...
        Version.PreReleaseLabel = PreRelease;

        if (!PreRelease.IsEmpty())
        {
            TArray<FString> PreReleaseParts;
            PreRelease.ParseIntoArray(PreReleaseParts, TEXT("."));
            const FString LastPart = PreReleaseParts.Num() > 0 ? PreReleaseParts.Last() : PreRelease;
            if (LastPart.IsNumeric())
            {
                Version.PreReleaseNumber = FCString::Atoi(*LastPart);
            }
        }
    ...
    FString GetPreReleasePrefix() const
    {
        ...
        TArray<FString> Parts;
        PreReleaseLabel.ParseIntoArray(Parts, TEXT("."));
        if (Parts.Num() > 1 && Parts.Last().IsNumeric())
        {
            Parts.Pop();
    ...
    FString ToString() const
    {
        ...
        FString VersionString = FString::Printf(TEXT("%d.%d.%d"), Major, Minor, Patch);
        if (!PreReleaseLabel.IsEmpty())
        {
            VersionString += TEXT("-") + PreReleaseLabel;
        }
    ...
    bool operator<(const FSemanticVersion &Other) const
    {
        ...
        const bool bThisPreRelease = !PreReleaseLabel.IsEmpty();
        const bool bOtherPreRelease = !Other.PreReleaseLabel.IsEmpty();
        if (bThisPreRelease != bOtherPreRelease)
            return bThisPreRelease;
        ...
        const FString ThisPrefix = GetPreReleasePrefix();
        const FString OtherPrefix = Other.GetPreReleasePrefix();
        ...
        return PreReleaseLabel < Other.PreReleaseLabel;
    }
