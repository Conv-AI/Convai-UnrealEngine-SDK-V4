...
public:
     ...
     *
     * Separate from OnFailure because the two severities are separate on the wire and
     * only one of them ends the session: an `error` packet does, an `error-response`
     * does not. A healthy connected session receives three `error-response` advisories
     * (FINDINGS F9), so treating them as failures would report a failure on every
     * successful connection. Fatal errors still reach OnFailure, by default here.
     */
