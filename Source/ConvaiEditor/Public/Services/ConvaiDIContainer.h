 ...
 *
 * NOT exported. Every member is defined inline here, so each translation unit gets its own; marking
 * an unspecialised class template with the module's API macro instead makes MSVC treat those members
 * as dllimport in every other module and look for them in this DLL, where only the instantiations
 * this module happens to use exist. Any other module resolving a service type of its own then fails
 * to link on TConvaiResult<TSharedPtr<ItsOwnInterface>>::GetValue.
 */
template <typename T>
class TConvaiResult
{
public:
    ...
    /** Add metadata entry */
    // Param renamed `Value` -> `InValue` to avoid shadowing the
    // TConvaiResult<T>::Value field (clang -Werror=shadow under UE 5.8).
    TConvaiResult<T> WithMetadata(const FString &Key, const FString &InValue) const
    {
        checkf(!bSuccess, TEXT("Cannot add metadata to successful result"));

        TConvaiResult<T> NewResult;
        NewResult.Value = InValue;
        NewResult.ErrorMessage = ErrorMessage;
        NewResult.bSuccess = false;
        NewResult.ErrorCode = ErrorCode;
        NewResult.ErrorContext = ErrorContext;
