...
enum class EConnectionEntryState : uint8
{
	...
	Active,
	Orphaned,
	/** The owner is alive and its session dropped; a retry is pending or the
	 *  session is Dormant. No expiry: only the owner's own attempt reclaims it,
	 *  and it is released when the chain ends. */
	Suspended
};
...
private:
	...
	void Shutdown();
	/** The session on DroppedProxy ended and the coordinator has decided what
	 *  comes next. A lease on any other proxy is not this drop's business. */
	void OnServerDisconnected(const UConvaiConnectionSessionProxy* DroppedProxy, EConvaiReconnectState After);

	/** A retry is about to reopen Proxy: true when the lease is its to use -
	 *  reclaimed from Suspended, or re-adopted when nobody holds one. */
	bool ReclaimForReconnect(UConvaiConnectionSessionProxy* Proxy, const FString& CharacterID);

	/** An orphan is reused only while its own client is still the live one. */
	bool IsOrphanStillLive() const;
	...
	/** The Connect Epoch the orphaned or prepared session was left on. */
	uint64 LeaseEpoch = 0;
	friend class UConvaiSubsystem;
	// A unit rig has to put the manager into the state a live connect leaves
	// behind; there is no public way in, and inventing one would be a
