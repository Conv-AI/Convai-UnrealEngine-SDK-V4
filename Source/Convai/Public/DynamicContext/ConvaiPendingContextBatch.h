...
struct CONVAI_API FConvaiPendingContextBatch
{
	...
	/** Keeps every staged item but asks for no response to any of it: after a
	 *  long gap the moment it was meant to answer has passed. */
	void DowngradeToNever();
...
struct CONVAI_API FConvaiHeldContextLane
{
	...
	bool HasWork() const;
	void Clear();
	/** As FConvaiPendingContextBatch::DowngradeToNever. */
	void DowngradeToNever();
