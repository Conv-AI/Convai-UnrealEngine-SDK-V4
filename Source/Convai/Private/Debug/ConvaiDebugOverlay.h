...
private:
	...
	// One marker per registered object, keyed by component — plus one per NAMED
	...
	struct FMarker
	{
		...
		bool bCachedOnScreen = false;
		FVector CachedAnchor = FVector::ZeroVector;
	...
	int32 MarkerUpdateCursor = 0;
	int32 OcclusionTraceCursor = 0;

	// Over-head action queue (selected character) plus a separate, downward
	// history of terminal results. Keeping separate canvas slots means result
	// animation cannot move or reorder the live queue pointer.
	SConstraintCanvas::FSlot* QueueSlot = nullptr;
	TSharedPtr<SWidget> QueueRoot;
	TSharedPtr<SVerticalBox> QueueBox;
	SConstraintCanvas::FSlot* ActionResultsSlot = nullptr;
	TSharedPtr<SVerticalBox> ActionResultsBox;
	struct FActionResultRow
	{
		TSharedPtr<class SBorder> Row;
		TSharedPtr<class STextBlock> Text;
		FLinearColor Color = FLinearColor::White;
		float Lifetime = 0.0f;
		float Remaining = 0.0f;
	};
	TArray<FActionResultRow> ActionResultRows;
	TWeakObjectPtr<UConvaiChatbotComponent> ActionResultsOwner;
	void ResetActionResults(UConvaiChatbotComponent* NewOwner = nullptr);
