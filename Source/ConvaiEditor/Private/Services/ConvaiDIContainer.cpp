...
void FConvaiDIContainer::Clear()
{
    ...
    for (auto &ServicePair : ServiceDescriptors)
    {
        if (ServicePair.Value.IsValid() && ServicePair.Value->SingletonInstance.IsValid())
        {
            ...
            catch (const FString &ErrorMsg)
            {
                UE_LOG(LogConvaiEditor, Error, TEXT("FString exception during service shutdown '%s': %s"),
                       *ServicePair.Key.ToString(), *ErrorMsg);
            }
            catch (...)
            {
                ...
                checkf(false, TEXT("Unknown exception during service shutdown: %s"), *ServicePair.Key.ToString());
#endif
            }
