...
namespace ConvaiMultipart
{
	 ...
	 */
	CONVAI_API void AppendUtf8(CONVAI_HTTP_PAYLOAD_ARRAY_TYPE& DataToSend, const FString& Text);

	CONVAI_API void AppendFormField(CONVAI_HTTP_PAYLOAD_ARRAY_TYPE& DataToSend, const FString& Boundary, const FString& Name, const FString& Value);

	CONVAI_API void AppendFileField(
		CONVAI_HTTP_PAYLOAD_ARRAY_TYPE& DataToSend,
		const FString& Boundary,
		const FString& Name,
		const FString& FileName,
		const TArray<uint8>& FileBytes,
		const FString& ContentType = TEXT("application/octet-stream"));
}
...
public:
	...
	// Outcome diagnostics, set before HandleSuccess/HandleFailure broadcast so a native
	// completion handler can classify the result instead of parsing prose. ResponseCode is
	// 0 when no HTTP response arrived (connection, DNS, or local setup failure);
	// FailureResponseBody carries the server body only for a non-2xx completion.
	int32 ResponseCode = 0;
	FString FailureResponseBody;
};
