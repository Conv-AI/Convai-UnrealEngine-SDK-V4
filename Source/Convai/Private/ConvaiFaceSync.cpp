...
EC_LipSyncMode UConvaiFaceSyncComponent::GetLipSyncMode()
{
	return LipSyncMode;
}

bool UConvaiFaceSyncComponent::GeneratesFacialDataAsBlendshapes()
{
	EC_LipSyncMode Mode = GetLipSyncMode();
	return Mode == EC_LipSyncMode::BS_MHA || Mode == EC_LipSyncMode::BS_ARKit;
}

TArray<float> UConvaiFaceSyncComponent::ConvaiGetFacialData()
{
...
TArray<FString> UConvaiFaceSyncComponent::ConvaiGetFacialDataNames()
{
...
bool UConvaiFaceSyncComponent::CalculateFrameSelection(FFrameSelectionResult& OutResult)
{
	...
	// Subtract the total paused duration to maintain continuity
	...
	const float FrameDuration = MainSequenceBuffer.Duration / MainSequenceBuffer.AnimationFrames.Num();
...
void UConvaiFaceSyncComponent::AddFrameToRecordingBuffer(const FAnimationFrame& Frame, float Duration)
{
