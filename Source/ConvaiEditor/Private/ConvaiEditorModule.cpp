...
#include "SceneObjects/SConvaiSceneObjectsManager.h"
...
static const FName ConvaiSceneObjectsTabName(TEXT("ConvaiSceneObjects"));
...
void FConvaiEditorModule::RegisterMovementPointVisualizer()
{
    auto DoRegister = []()
    {
        if (GUnrealEd)
        {
            TSharedPtr<FComponentVisualizer> Visualizer = MakeShareable(new FConvaiMovementPointVisualizer());
            // RegisterComponentVisualizer calls OnRegister() itself.
            GUnrealEd->RegisterComponentVisualizer(UConvaiObjectComponent::StaticClass()->GetFName(), Visualizer);
        }
    };
    ...
    {
        // Module can load before UUnrealEdEngine is constructed — defer.
#if ENGINE_MAJOR_VERSION > 5 || (ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8)
        MovementPointVisualizerInitHandle = FCoreDelegates::GetOnPostEngineInit().AddLambda(DoRegister);
#else
        MovementPointVisualizerInitHandle = FCoreDelegates::OnPostEngineInit.AddLambda(DoRegister);
#endif
    }
}
...
{
    // Content browser context menu (Make Convai Movement) - supported on all engine versions
    FConvaiContentBrowserContextMenu::Register();

    // Viewport gizmo editing for ObjectEntry.MovementPoints on Convai Object
    // components - supported on all engine versions
    RegisterMovementPointVisualizer();

    // Note: the BP graph right-click "Create Convai Action Handler" entry is now
    // contributed via UK2Node_ConvaiCreateActionHandler::GetMenuActions — the
    // Blueprint Action Database discovers it automatically, no manual register needed.

#if (ENGINE_MAJOR_VERSION < 5) || (ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION < 2)
    // ConvaiEditor UI requires UE 5.2+ (FAppStyle, modern ToolMenus, FEditorDelegates::OnEditorInitialized in 5.3)
    UE_LOG(LogConvaiEditor, Log, TEXT("ConvaiEditor: Editor UI disabled - requires UE 5.2 or later"));
    return;
#endif
    ...
    // Struct-level customization for FConvaiObjectEntry: hides OptionalPositionVector
    // everywhere (deprecated, auto-written) and hides Ref when the outer is
    // UConvaiObjectComponent (Ref is auto-bound to GetOwner() there).
    PropertyEditor.RegisterCustomPropertyTypeLayout(
        TEXT("ConvaiObjectEntry"),
        FOnGetPropertyTypeCustomizationInstance::CreateStatic(&FConvaiObjectEntryCustomization::MakeInstance));
    // Custom picker for FConvaiTrackedProperty::PropertyPath. UE 5.5's
    // meta=(GetOptions=...) resolver fails when the property is inside a
    // struct that's inside a TArray, so we replace the FName field with
    // IPropertyAccessEditor::MakePropertyBindingWidget — the AnimGraph-style
    // hierarchical picker — rooted on the owning Actor's class.
    PropertyEditor.RegisterCustomPropertyTypeLayout(
        TEXT("ConvaiTrackedProperty"),
        FOnGetPropertyTypeCustomizationInstance::CreateStatic(&FConvaiTrackedPropertyCustomization::MakeInstance));
    ...
    if (!InitializeThemeSystem())
    {
        UE_LOG(LogConvaiEditor, Error, TEXT("ConvaiEditor: Editor UI disabled - style system failed to initialize"));
        return;
    }
    ...
    FGlobalTabmanager::Get()->RegisterNomadTabSpawner(
        ConvaiSceneObjectsTabName,
        FOnSpawnTab::CreateRaw(this, &FConvaiEditorModule::OnSpawnSceneObjectsTab))
        .SetDisplayName(LOCTEXT("ConvaiSceneObjectsTabTitle", "Convai Scene Objects"))
        .SetTooltipText(LOCTEXT(
            "ConvaiSceneObjectsTabTooltip",
            "Inspect loaded Convai objects, manage movement points, and safely remove editor-instance components."))
        .SetIcon(FSlateIcon(FConvaiStyle::GetStyleSetName(), "Convai.Icon.SceneObjects"))
...
{
    if (MovementPointVisualizerInitHandle.IsValid())
    {
#if ENGINE_MAJOR_VERSION > 5 || (ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8)
        FCoreDelegates::GetOnPostEngineInit().Remove(MovementPointVisualizerInitHandle);
#else
        FCoreDelegates::OnPostEngineInit.Remove(MovementPointVisualizerInitHandle);
#endif
        MovementPointVisualizerInitHandle.Reset();
    }
    if (GUnrealEd)
    {
        GUnrealEd->UnregisterComponentVisualizer(UConvaiObjectComponent::StaticClass()->GetFName());
    }
    ...
    FGlobalTabmanager::Get()->UnregisterNomadTabSpawner(ConvaiSceneObjectsTabName);
...
void FConvaiEditorModule::OpenSceneObjectsManager()
{
    FGlobalTabmanager::Get()->TryInvokeTab(ConvaiSceneObjectsTabName);
}

TSharedRef<SDockTab> FConvaiEditorModule::OnSpawnSceneObjectsTab(
    const FSpawnTabArgs &SpawnTabArgs)
{
    return SNew(SDockTab)
        .TabRole(ETabRole::NomadTab)
        [
            SNew(SConvaiSceneObjectsManager)
        ];
}
...
bool FConvaiEditorModule::InitializeThemeSystem()
{
    IConvaiDIContainer &DIContainer = FConvaiDIContainerManager::Get();
    ...
    {
        UE_LOG(LogConvaiEditor, Error, TEXT("Failed to resolve ThemeManager - Error: %s"), *ThemeResult.GetError());
        return false;
    }
    ...
    {
        UE_LOG(LogConvaiEditor, Error, TEXT("Failed to load theme '%s' and default theme fallback"), *ThemeId);
        return false;
    }

    FConvaiStyle::Initialize(nullptr);
    auto StyleRegistryResult = DIContainer.Resolve<IConvaiStyleRegistry>();
    if (StyleRegistryResult.IsFailure() || !StyleRegistryResult.GetValue().IsValid() || !StyleRegistryResult.GetValue()->IsInitialized())
    {
        UE_LOG(LogConvaiEditor, Error, TEXT("Failed to initialize Convai style registry"));
        return false;
    }

    FConvaiWidgetFactory::Initialize();
    bStyleSystemInitialized = true;
    return true;
}
...
void FConvaiEditorModule::RegisterEditorMenu()
{
    ...
    FToolMenuSection &ToolsSection = ToolsMenu->FindOrAddSection("Convai");
    ToolsSection.Label = LOCTEXT("ConvaiToolsHeader", "Convai");
#else
    FToolMenuSection &ToolsSection = ToolsMenu->FindOrAddSection(
        "Convai", LOCTEXT("ConvaiToolsHeader", "Convai"));
#endif
    ToolsSection.AddMenuEntry(
        "Convai.SceneObjects",
        LOCTEXT("OpenSceneObjectsLabel", "Scene Objects"),
        LOCTEXT(
            "OpenSceneObjectsTooltip",
            "Inspect loaded Convai objects, manage movement points, and safely remove editor-instance components."),
        FSlateIcon(FConvaiStyle::GetStyleSetName(), "Convai.Icon.SceneObjects"),
        FUIAction(FExecuteAction::CreateRaw(this, &FConvaiEditorModule::OpenSceneObjectsManager)));
}
...
void FConvaiEditorModule::ShowWelcomeWindowIfNeeded()
{
    ...
    {
        if (!bStyleSystemInitialized || !FConvaiStyleResources::IsInitialized())
        {
            UE_LOG(LogConvaiEditor, Error, TEXT("Cannot open welcome window - style system not initialized"));
            return;
        }
...
void FConvaiEditorModule::OnEditorInitialized(double DeltaTime)
{
    ...
    // If the user requested MCP setup last session, finish it now that the editor
    // and the MCP/Terminal modules are loaded (no-op if nothing is pending).
    FConvaiMcpSetupService::ApplyPendingSetupIfAny();
#endif
