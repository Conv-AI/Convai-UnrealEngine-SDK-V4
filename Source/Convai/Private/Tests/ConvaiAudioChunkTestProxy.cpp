...
UConvaiAudioChunkTestProxy* UConvaiAudioChunkTestProxy::CreateContinuousAudioStreamProxy(
	UObject* WorldContextObject,
	UConvaiAudioStreamer* AudioStreamer,
	USoundWave* AudioToPlay,
	...
	float IntervalSeconds)
{
	...
	if (!AudioToPlay)
	{
		UE_LOG(ConvaiAudioChunkTestLog, Error, TEXT("CreateContinuousAudioStreamProxy: AudioToPlay is null"));
		return nullptr;
	}

	// Ensure minimum values
	ChunkDurationSeconds = FMath::Max(0.1f, ChunkDurationSeconds);
	IntervalSeconds = FMath::Max(0.0f, IntervalSeconds);

	// Calculate how many chunks we need based on audio duration
	float AudioDuration = AudioToPlay->Duration;
	int32 NumChunks = FMath::CeilToInt(AudioDuration / ChunkDurationSeconds);

	// Ensure at least one chunk
	NumChunks = FMath::Max(1, NumChunks);

	// Build arrays with uniform chunk durations and intervals
	TArray<float> ChunkDurations;
	TArray<float> Delays;
	...
	for (int32 i = 0; i < NumChunks; ++i)
	{
		...
		// First chunk has no delay, subsequent chunks have the specified interval
		Delays.Add(i == 0 ? 0.0f : IntervalSeconds);
	}

	UE_LOG(ConvaiAudioChunkTestLog, Log, TEXT("CreateContinuousAudioStreamProxy: Audio=%.2fs, ChunkDuration=%.2fs, Interval=%.2fs, NumChunks=%d"),
		AudioDuration, ChunkDurationSeconds, IntervalSeconds, NumChunks);
...
void UConvaiAudioChunkTestProxy::SendNextChunk()
{
	...
	UE_LOG(ConvaiAudioChunkTestLog, Log, TEXT("Sending chunk %d: %.3fs (%d bytes) at offset %d"),
		CurrentChunkIndex, ChunkDuration, ChunkBytes, CurrentByteOffset);
...
{
	UE_LOG(ConvaiAudioChunkTestLog, Log, TEXT("Chunked audio test completed: sent %d chunks, %d bytes total"),
		CurrentChunkIndex, CurrentByteOffset);
