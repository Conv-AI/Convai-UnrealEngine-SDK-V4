...
public:
	...
	/**
	 * Shared poll cadence (seconds) used by UConvaiSubsystem to evaluate all
	 * registered UConvaiObjectComponents on the same clock. Resolved via the
	 * custom-param chain (hardcoded default -> UConvaiSettings::CustomPrams ->
	...
	/**
	 * Project-wide default for UConvaiChatbotComponent::ContextDebounceWindow (seconds).
	 * Used at construction time only; the per-component UPROPERTY still overrides
	 * this on already-serialized instances.
	 */
	UFUNCTION(BlueprintPure, Category = "Convai|Utilities")
	static float GetContextDebounceWindowDefault();

	/**
	 * Project-wide default for UConvaiChatbotComponent::ContextMaxDebounceWindow (seconds).
	 * Used at construction time only; the per-component UPROPERTY still overrides
	 * this on already-serialized instances.
	 */
	UFUNCTION(BlueprintPure, Category = "Convai|Utilities")
