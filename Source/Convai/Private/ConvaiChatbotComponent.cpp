...
{
	OutActions = EnvironmentData.Actions;
	if (bEnableRemindSelfAction && IndexOfActionByName(OutActions, RemindSelfActionName) == INDEX_NONE)
	{
		OutActions.Add(BuildRemindSelfAction());
	}
	if (bEnableWatchPropertyAction && IndexOfActionByName(OutActions, WatchPropertyActionName) == INDEX_NONE)
	{
...
FString UConvaiChatbotComponent::GetActionConfigJson()
{
	...
	// dispatch consults ConnectedBuiltInNames — never the live toggles — so a
	// mid-session toggle flip can't stall an already-advertised action's queue
	// (or activate an unadvertised one). Toggle edits apply on reconnect, like
	// every other action mutation.
	...
	if (bEnableRemindSelfAction && IndexOfActionByName(EnvironmentData.Actions, RemindSelfActionName) == INDEX_NONE)
	{
		ConnectedBuiltInNames.Add(RemindSelfActionName);
	}
	if (bEnableWatchPropertyAction && IndexOfActionByName(EnvironmentData.Actions, WatchPropertyActionName) == INDEX_NONE)
	{
...
void UConvaiChatbotComponent::SyncBuiltInActionContextFacts()
{
	const auto WasAdvertised = [this](const TCHAR* ActionName)
	{
	...
	const auto SyncFact = [this](bool bWasAdvertised, const TCHAR* Key, const TCHAR* Sentence)
	{
		if (bWasAdvertised)
		{
			SetContextFact(Sentence, Key, EC_RunLLMOption::Never);
		}
		else
		{
			RemoveContextFact(Key);
		}
	};

	SyncFact(WasAdvertised(RemindSelfActionName), RemindSelfContextFactKey, RemindSelfContextFact);
	SyncFact(WasAdvertised(WatchPropertyActionName), WatchPropertyContextFactKey, WatchPropertyContextFact);
}
	...
	const bool bWasInterrupted, const bool bWasAborted, const FString& ErrorReason)
{
	...
	AsyncTask(ENamedThreads::GameThread,
		[WeakSelf, ResponseId, bWasInterrupted, bWasAborted, ErrorReason, bApplyToCurrentResponse]
		{
			if (UConvaiChatbotComponent* Self = WeakSelf.Get())
			{
				if (bApplyToCurrentResponse)
...
void UConvaiChatbotComponent::StartSession()
{
	...
	// advertised. Never-response facts join the initial context batch without
	// waking the model, while stable keys remove stale hints on later sessions.
	SyncBuiltInActionContextFacts();
