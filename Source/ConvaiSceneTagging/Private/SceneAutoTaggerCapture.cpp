	...
	const bool bAllowSuppressedSpecularRecovery)
{
	...
	// Authored scene lighting is always the first choice. These transient lights
	// remain off unless a measured one-shot capture is genuinely unreadable.
	...
	const bool bAllowSuppressedSpecularRecovery)
{
	...
	// Merely position the hidden emergency fill. The baseline capture uses the
	// authored level lights; a measured dark-result recovery may enable it later.
...
float ResolveWorldAutoExposureBias(UWorld* World, const FVector& ViewLocation)
{
	...
	for (IInterface_PostProcessVolume* VolumePtr : World->PostProcessVolumes)
	{
		if (!VolumePtr)
		{
			continue;
		}
		IInterface_PostProcessVolume& Volume = *VolumePtr;
#else
	...
	const bool bUseLumenReflections)
{
	...
	// Scene captures blend post-process volumes at their own camera location.
	// Retain the authored lighting/color pipeline, while excluding camera effects
	// that are unstable or misleading for an isolated orbit camera.
	...
	const bool bSuppressDirectSpecularHighlights)
{
	...
	// Let world post-process volumes provide the same exposure and color treatment
	// as the authored scene. Recovery probes may temporarily override only EV.
	...
	const bool bUseLumenGlobalIllumination = RendererSettings
		&& RendererSettings->DynamicGlobalIllumination == EDynamicGlobalIlluminationMethod::Lumen;
	const bool bUseLumenReflections = !bSuppressDirectSpecularHighlights
		&& RendererSettings
	...
	const bool bRenderFromCallerTick)
{
	...
	// Capture remains fully usable with authored scene lighting if the optional
	// dark-result fallback cannot be allocated.
	...
	TArray<uint8>* OutGeometryMask)
{
	...
	// Preserve authored scene lighting first. A bounded EV-only retry handles
	...
	if (NeedsReadabilityRecovery(RetainedEvaluation))
	{
		// Test the bounded neutral fill against the authored exposure baseline. Do
