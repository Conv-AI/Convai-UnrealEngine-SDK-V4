...
void UConvaiPlayerComponent::StopVoiceChunkCapture()
{
	...
	{
		// Stream voice data
		AddPCMDataToSend(TArray<uint8>(reinterpret_cast<uint8*>(OutConverted.GetData()), OutConverted.Num() * sizeof(int16)), false, ConvaiConstants::VoiceCaptureSampleRate, 1);
	}
}
...
void UConvaiPlayerComponent::OnParticipantConnected(FString ParticipantId)
{
	OnParticipantConnectionStateChangedEvent.Broadcast(this, ParticipantId, EC_ConnectionState::Connected);
}

void UConvaiPlayerComponent::OnParticipantDisconnected(FString ParticipantId)
{
