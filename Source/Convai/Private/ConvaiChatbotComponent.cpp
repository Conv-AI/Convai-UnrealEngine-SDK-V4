...
void UConvaiChatbotComponent::UpdateContext(const FString& Text, EC_ContextUpdateMode Mode, EC_RunLLMOption RunLLM)
{
	if (IsValid(SessionProxyInstance))
	{
		SessionProxyInstance->UpdateContext(Text, Mode, RunLLM);
	}
}

// ── Dynamic Context ──────────────────────────────────────────────

void UConvaiChatbotComponent::SetContextState(const FString& Name, const FString& Value, EC_RunLLMOption RunLLM)
{
	FString OldValue;
	const bool bExisted = DynamicContextTracker.SetState(Name, Value, &OldValue);

	if (RunLLM == EC_RunLLMOption::Never)
	{
		// Rebuild and replace the full canonical context, no LLM trigger
		UpdateContext(DynamicContextTracker.BuildCanonicalContext(),
			EC_ContextUpdateMode::Replace, EC_RunLLMOption::Never);
	}
	else // Auto or Always
	{
		if (bExisted)
		{
			// First replace canonical context so the LLM sees the latest values
			UpdateContext(DynamicContextTracker.BuildCanonicalContext(),
				EC_ContextUpdateMode::Replace, EC_RunLLMOption::Never);
			// Then append a human-readable change message to trigger the LLM
			UpdateContext(FString::Printf(TEXT("%s changed from %s to %s"), *Name, *OldValue, *Value),
				EC_ContextUpdateMode::Append, RunLLM);
		}
		else
		{
			// Brand new state — just append it
			UpdateContext(FString::Printf(TEXT("%s is %s"), *Name, *Value),
				EC_ContextUpdateMode::Append, RunLLM);
		}
	}
}

void UConvaiChatbotComponent::SetContextStates(const TMap<FString, FString>& States, EC_RunLLMOption RunLLM)
{
	// First pass: update all tracked values, collecting old values and existence info
	struct FStateChange
	{
		FString Name;
		FString NewValue;
	...
	TArray<FStateChange> Changes;
	Changes.Reserve(States.Num());

	bool bAnyExisted = false;
	for (const auto& Pair : States)
	{
		FStateChange Change;
		Change.Name = Pair.Key;
		Change.NewValue = Pair.Value;
		Change.bExisted = DynamicContextTracker.SetState(Pair.Key, Pair.Value, &Change.OldValue);
		bAnyExisted |= Change.bExisted;
	...
	else // Auto or Always
	{
		if (bAnyExisted)
		{
			// Replace canonical context first so the LLM sees the latest values
			UpdateContext(DynamicContextTracker.BuildCanonicalContext(),
				EC_ContextUpdateMode::Replace, EC_RunLLMOption::Never);
		}

		// Build combined change message
		TArray<FString> Lines;
		Lines.Reserve(Changes.Num());
		for (const FStateChange& Change : Changes)
		{
			if (Change.bExisted)
			{
				Lines.Add(FString::Printf(TEXT("%s changed from %s to %s"),
					*Change.Name, *Change.OldValue, *Change.NewValue));
			}
			else
			{
				Lines.Add(FString::Printf(TEXT("%s is %s"), *Change.Name, *Change.NewValue));
			}
		}
		UpdateContext(FString::Join(Lines, TEXT("\n")),
			EC_ContextUpdateMode::Append, RunLLM);
	}
}

void UConvaiChatbotComponent::AddContextEvent(const FString& Text, EC_RunLLMOption RunLLM)
{
	DynamicContextTracker.AddEvent(Text);
	UpdateContext(Text, EC_ContextUpdateMode::Append, RunLLM);
}
