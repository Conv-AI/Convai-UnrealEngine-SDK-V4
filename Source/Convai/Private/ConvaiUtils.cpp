...
UConvaiSubsystem* UConvaiUtils::GetConvaiSubsystem(const UObject* WorldContextObject)
{
	...
	// A unit rig has no world at all: it constructs the subsystem directly and
	// outers the objects under test to it. Walking the outer chain is how they
	// find it. Production callers all have a world, so this costs them one
	// null check.
	if (!WorldContextObject->GetWorld())
	{
		for (UObject* Outer = WorldContextObject->GetOuter(); Outer; Outer = Outer->GetOuter())
		{
			if (UConvaiSubsystem* OuterSubsystem = Cast<UConvaiSubsystem>(Outer))
			{
				return OuterSubsystem;
			}
		}
	}
...
FString UConvaiUtils::GetVisionCharacterID()
{
	FString CharacterID = Convai::Get().GetConvaiSettings()->VisionCharacterID;
	CharacterID.TrimEndInline();
	CharacterID.TrimStartInline();

	FString CommandLineCharacterID = UCommandLineUtils::GetCommandLineFlagValueAsString(TEXT("ConvaiVisionCharacterID"), TEXT(""));
	if (!CommandLineCharacterID.IsEmpty())
	{
		CharacterID = CommandLineCharacterID;
	}

	// Vision shipped before it had a setting of its own; projects that only ever
	// filled in the debug test character keep working until they set this one.
	return CharacterID.IsEmpty() ? GetTestCharacterID() : CharacterID;
}
...
int32 UConvaiUtils::GetReconnectAttempts()
{
	// Project Settings, with a command-line override for tests. Read through
	// the typed property rather than the string map: this one is a real
	// setting a customer sets in the editor, not a custom param.
	int32 Attempts = 0;
	if (const UConvaiSettings* Settings = Convai::Get().GetConvaiSettings())
	{
		Attempts = Settings->ReconnectAttempts;
	}

	const FString Override = UCommandLineUtils::GetCommandLineFlagValueAsString(
		TEXT("ReconnectAttempts"), FString());
	if (!Override.IsEmpty())
	{
		Attempts = FCString::Atoi(*Override);
	}
	return FMath::Clamp(Attempts, 0, 10);
}

EC_IdleTimeoutBehavior UConvaiUtils::GetIdleTimeoutBehavior()
{
	EC_IdleTimeoutBehavior Choice = EC_IdleTimeoutBehavior::ReopenWhenPlayerReturns;
	if (const UConvaiSettings* Settings = Convai::Get().GetConvaiSettings())
	{
		Choice = Settings->IdleTimeoutBehavior;
	}

	const FString Override = UCommandLineUtils::GetCommandLineFlagValueAsString(
		TEXT("IdleTimeoutBehavior"), FString());
	if (!Override.IsEmpty())
	{
		const int64 Value = StaticEnum<EC_IdleTimeoutBehavior>()->GetValueByNameString(Override);
		if (Value != INDEX_NONE)
		{
			Choice = static_cast<EC_IdleTimeoutBehavior>(Value);
		}
	}
	return Choice;
}
