...
void UConvaiSubsystem::Deinitialize()
{
    ...
    {
        World->GetTimerManager().ClearTimer(MicWatchdogTimerHandle);
    }
...
void UConvaiSubsystem::ResolveDisconnectReason()
{
    // A disconnect the game asked for keeps its reason: that was decided before
    // the transport went down, and nothing observed here can override it.
    if (LastDisconnectReason == EC_DisconnectReason::Explicit)
    {
        return;
    }

    LastDisconnectReason = bIdleDisconnectPending
        ? EC_DisconnectReason::Idle
        : EC_DisconnectReason::Unexpected;
}
...
void UConvaiSubsystem::MicWatchdogScheduleCheck(double DelaySeconds)
{
    UWorld* World = GetWorld();
    if (!World)
    {
    ...
    World->GetTimerManager().SetTimer(MicWatchdogTimerHandle, this, &UConvaiSubsystem::MicWatchdogCheck,
        static_cast<float>(DelaySeconds), false);
}

void UConvaiSubsystem::MicWatchdogCheck()
{
    const double Quiet = FPlatformTime::Seconds() - MicWatchdogLastSpeechTime.load();
    if (Quiet < MicWatchdogSilenceSeconds)
    {
        MicWatchdogScheduleCheck(MicWatchdogSilenceSeconds - Quiet);
        return;
    }
        ...
        if (Rms >= FMath::Max(MicWatchdogSpeechRms, Floor * MicWatchdogSpeechOverFloor))
        {
            ...
            if (SpeechFrames >= MicWatchdogMinSpeechFrames && !bMicWatchdogArmed.exchange(true))
            {
                ...
                AsyncTask(ENamedThreads::GameThread, [WeakThis]()
                {
                    ...
                    {
                        Subsystem->MicWatchdogScheduleCheck(MicWatchdogSilenceSeconds);
                    }
                });
            }
        }
...
void UConvaiSubsystem::ForceUserStoppedSpeaking(const UConvaiConnectionSessionProxy* SessionProxy) const
{
    ...
    ConvaiClient->SendMessageWithLabel(ConvaiConstants::WebRTC::label::Default, ConvaiConstants::WebRTC::MessageType::ForceUserStoppedSpeaking, TCHAR_TO_UTF8(TEXT("{}")));
    CONVAI_LOG(ConvaiSubsystemLog, Log, TEXT("Sent force-user-stopped-speaking"));
}
...
void UConvaiSubsystem::OnConnectionFailed()
{
    ...
    AsyncTask(ENamedThreads::GameThread, [WeakSubsystem]()
    {
        ...
        {
            ValidSubsystem->ResolveDisconnectReason();
...
void UConvaiSubsystem::OnConnectedToServer(const char* session_id, const char* char_session_id)
{
    ...
    AsyncTask(ENamedThreads::GameThread, [WeakThis]()
    {
        ...
        if (UConvaiSubsystem* Subsystem = WeakThis.Get())
        {
            // A fresh session starts the AFK clock now: a player who never says
            // anything is measured from connect, not from the epoch.
            Subsystem->LastPlayerActivityTime = FPlatformTime::Seconds();
            Subsystem->bIdleDisconnectPending = false;
            Subsystem->bSpentTalkingGrace = false;
            Subsystem->LastDisconnectReason = EC_DisconnectReason::Unexpected;
...
void UConvaiSubsystem::OnDisconnectedFromServer()
{
    ...
    AsyncTask(ENamedThreads::GameThread, [WeakThis, AttendeesToDisconnect]()
    {
        if (UConvaiSubsystem* Subsystem = WeakThis.Get())
        {
            ...
            Subsystem->ResolveDisconnectReason();
...
void UConvaiSubsystem::OnDataPacketReceived(const char* JsonData, const char* attendee_id)
{
    ...
    switch (DataPacketType)
    {
        ...
        case EC_PacketType::BotLLMText:
            {
                // Recognised and discarded until 2026-08-20, which is why a
                // text prompt produced no character transcript anywhere: no
                // bot-transcription packet is sent on that path, so this is
                // the only carrier the answer has (FINDINGS F23).
                FString Text;
                if (DataObj.IsValid() && GetStringSafe(DataObj, TEXT("text"), Text)
                    && !Text.IsEmpty())
                {
                    OnBotLLMText(Text);
                }
...
void UConvaiSubsystem::OnBotTranscript(const char* text, const char* attendee_id) const
{
    ...
    {
        // Authoritative for the turn: one packet carrying the whole utterance,
        // against the token stream assembled from bot-llm-text.
        FScopeLock Lock(&BotTurnTextLock);
        BotTurnText = Transcript;
    }

    // Forward to the current character session
    if (const TScriptInterface<IConvaiConnectionInterface> Interface = CurrentCharacterSession->GetConnectionInterface(); Interface.GetObject())
    {
        Interface->OnTranscriptionReceived(Transcript, true, false);
    }
}
...
void UConvaiSubsystem::OnBotLLMText(const FString& Text) const
{
    if (!IsValid(CurrentCharacterSession))
    {
        return;
    ...
    // The answer so far, not the token: a consumer showing one packet at a
    // time would render "Hello" and then "," on its own. Not final -- the turn
    // is still being written, and bot-llm-stopped is what settles it.
    if (const TScriptInterface<IConvaiConnectionInterface> Interface = CurrentCharacterSession->GetConnectionInterface(); Interface.GetObject())
    {
        Interface->OnTranscriptionReceived(Running, true, false);
    }
}
...
void UConvaiSubsystem::OnBotLLMStopped(const char* attendee_id) const
{
    ...
    if (const TScriptInterface<IConvaiConnectionInterface> Interface = CurrentCharacterSession->GetConnectionInterface(); Interface.GetObject())
    {
        ...
        // The turn's text, not "". The empty final was the plugin's own doing,
        // not the server's, and it meant the only final transcription a game
        // ever received for the character was blank -- so anything binding
        // this delegate to show subtitles cleared them a moment after the
        // character spoke, and anything keyed on final saw nothing at all
        // (FINDINGS F23).
        FString Spoken;
        {
            FScopeLock Lock(&BotTurnTextLock);
            Spoken = MoveTemp(BotTurnText);
            BotTurnText.Reset();
        }
        Interface->OnTranscriptionReceived(Spoken, true, true);
    }
}
