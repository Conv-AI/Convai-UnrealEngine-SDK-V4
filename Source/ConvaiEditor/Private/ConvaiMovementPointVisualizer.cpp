...
namespace
{
	...
	constexpr float RingVisualScale = 0.5f;
...
void FConvaiMovementPointVisualizer::DrawVisualization(const UActorComponent* Component, const FSceneView* View, FPrimitiveDrawInterface* PDI)
{
	...
	for (int32 i = 0; i < Entry.MovementPoints.Num(); ++i)
	{
		...
		// Context: a compact visual cue for the effective acceptance radius
		// (lifted a touch off the ground) and a tether to the owning object.
		// The resolver still uses its full, unscaled runtime tolerance.
		DrawCircle(PDI, Location + FVector(0.0f, 0.0f, RingLift), FVector::ForwardVector,
			FVector::RightVector, Color,
			FMath::Max(Entry.AcceptanceRadius, 150.0f) * RingVisualScale,
			32, SDPG_World, 1.0f);
