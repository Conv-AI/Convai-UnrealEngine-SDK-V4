...
{
	// The one place the main thread deliberately waits on the network: a clean
	// Disconnect is what frees the server's concurrency slot, and at process
	// exit there is no later chance to do it. Bounded, and a still-running
	// teardown is left alone rather than unloading the library under it.
	FConvaiConnectReaper::Shutdown(10.0);
