...
void UConvaiChatbotComponent::StopSession()
{
	...
	if (IsValid(SessionProxyInstance))
	{
		// The disconnect verdict is global to the subsystem, so only a chatbot that
		// actually held a session may set it — otherwise tearing down any other
		// chatbot suppresses the reconnect for the one that is still talking.
		if (UConvaiSubsystem* ConvaiSubsystem = UConvaiUtils::GetConvaiSubsystem(this))
		{
