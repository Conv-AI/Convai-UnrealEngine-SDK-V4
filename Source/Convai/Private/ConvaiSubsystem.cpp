...
uint32 FConvaiConnectionThread::Run()
{
    if (ConnectionParams.Client)
    {
        ...
        const int32 InvocationClientVersionLen = SafeConvertToUTF8(InvocationClientVersionBuffer, BUFFER_SIZE, *PluginVersionName);
        ...
        if (InvocationSourceLen >= 0 && InvocationClientVersionLen >= 0 && InvocationExtraMetadataLen >= 0)
        {
            ...
            // User will add there app version here
            InvocationMetadata.client_version = "";
            ...
            CONVAI_LOG(ConvaiSubsystemLog, Log, TEXT("InvocationMetadata - ExtraMetadata: %s"), *ExtraMetadataString);
        }
        else
        {
...
void UConvaiSubsystem::UpdateContext(const UConvaiConnectionSessionProxy* SessionProxy, const FString& Text, EC_ContextUpdateMode Mode, EC_RunLLMOption RunLLM) const
{
    if (!IsValid(SessionProxy) || !ConvaiClient || !bIsConnected)
    {
        return;
    ...
    DataJson->SetStringField(TEXT("run_llm"), UTF8_TO_TCHAR(RunLLMOptionToString(RunLLM)));
...
void UConvaiSubsystem::OnConnectedToServer(const char* session_id, const char* char_session_id)
{
    ...
    CONVAI_LOG(ConvaiSubsystemLog, Log, TEXT("SessionID: %s, CharSessionID: %s"), *SessionID, *CharSessionID);
