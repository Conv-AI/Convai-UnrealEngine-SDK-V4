...
struct CONVAI_API FConvaiAction
{
	...
	/** Editable wire-format preview. Two-way sync with the structured fields above:
	 *  edit either side, the other follows. Parse failures here silently revert.
	 *  Persisted (not Transient) — Transient on a nested struct UPROPERTY interacted
	 *  badly with UE's array-element context menu (right-click → Delete was blocked).
	 *  PostEditChangeProperty / PostLoad keep it consistent, so the duplicated storage
	 *  is harmless. */
	UPROPERTY(EditAnywhere, category = "Convai|Action API",
		meta = (MultiLine = true,
				ToolTip = "Live wire-format preview of this action. Edit either this or the structured fields above; the other side syncs automatically."))
