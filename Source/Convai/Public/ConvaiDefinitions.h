...
public:
	...
	void SetEmotionDataSingleEmotion(const FString& EmotionRespponse, float EmotionOffset)
	{
		...
		// @TODO : We should remove this when backend stops spamming neutral emotion
		if(Emotion == EBasicEmotions::None)
		{
			return;
		}
...
/**
 * Why a session ended, beside the reason.
 *
 * The reason stays at three values because Blueprints switch on it and a new
 * value would break every one of those graphs. The cause is the detail, and it
 * is what decides whether retrying can help: Quota and Preflight never can,
 * Network and ServerClosed usually can.
 */
UENUM()
enum class EC_DisconnectCause : uint8
{
	/** Nothing more is known. Everything is this until phase 3 classifies the
	 *  transport's reason string and the /connect status. */
	Unknown UMETA(DisplayName = "Unknown"),
	Network UMETA(DisplayName = "Network"),
	ServerClosed UMETA(DisplayName = "Server Closed"),
	/** The character's own participant left a transport that stayed up. */
	BotLeft UMETA(DisplayName = "Bot Left"),
	/** /connect answered, and said no. */
	ConnectRejected UMETA(DisplayName = "Connect Rejected"),
	Auth UMETA(DisplayName = "Auth"),
	Quota UMETA(DisplayName = "Quota"),
	PipelineError UMETA(DisplayName = "Pipeline Error"),
	/** Too many abandoned attempts are still being torn down. */
	TransportBlocked UMETA(DisplayName = "Transport Blocked"),
	/** Never reached the server: no API key, no character id, wrong ABI. */
	Preflight UMETA(DisplayName = "Preflight"),
	/** A newer Start Session for a different character took the connection. */
	Superseded UMETA(DisplayName = "Superseded"),
};

/** What a session the server closed for inactivity does next. Ignored while
 *  Reconnect Attempts is 0. */
UENUM()
enum class EC_IdleTimeoutBehavior : uint8
{
	/** Stays closed until the player speaks, types or presses to talk, then
	 *  reopens. Holds no server session while nobody is there. */
	ReopenWhenPlayerReturns UMETA(DisplayName = "Reopen When Player Returns"),
	/** Reopens straight away, so the character answers instantly when the
	 *  player comes back: once per idle close, at most once every 120 s, then
	 *  as Reopen When Player Returns. Holds a concurrency slot meanwhile. */
	ReopenImmediately UMETA(DisplayName = "Reopen Immediately"),
	/** Ends the session for good; only a new Start Session opens another. */
	StayClosed UMETA(DisplayName = "Stay Closed"),
};
...
/** Where a reconnect is. One chain - from a drop to a session that works, or
 *  to giving up - ends in exactly one of Reconnected, Gave Up or Dormant. */
enum class EC_ReconnectStatus : uint8
{
	/** A retry is waiting out its backoff. */
	Scheduled,
	/** A retry is connecting now. */
	Attempting,
	/** A retry reached a character that is ready. */
	Reconnected,
	/** No more retries: the budget ran out, the cause cannot pass, or the game stopped it. */
	GaveUp,
	/** Nobody is here to reconnect for; the player's next activity brings the character back. */
	Dormant,
};

struct FConvaiReconnectStatus
{
	EC_ReconnectStatus Status = EC_ReconnectStatus::Scheduled;

	/** This chain's attempt, from 1. */
	int32 Attempt = 0;

	int32 MaxAttempts = 0;

	/** Scheduled only: how long until the attempt. */
...
public:
	...
	/** Who to tell when this attempt fails, and which attempt it was.
	 *
	 *  A failure used to be reported through a static that looked up "the
	 *  first game world's subsystem", which is the wrong instance under PIE
	 *  multi-client and carried no way to tell a failure of the attempt in
	 *  flight from one of an attempt already abandoned. */
	TWeakObjectPtr<class UConvaiSubsystem> OwningSubsystem;
	uint64 ConnectEpoch = 0;
	...
	/** Reports this attempt's failure to the subsystem that started it, stamped
	 *  with the epoch it started in. Safe from the connection thread. */
	void ReportAttemptFailed() const;
};
...
struct FConvaiPersonalityTraits
{
    ...
    UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Convai")
    int32 Extraversion = 0;

    UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Convai")
    int32 Agreeableness = 0;

    UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Convai")
    int32 Meticulousness = 0;
};
...
struct FConvaiAvatarInfo
{
    ...
    UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Convai")
    bool bEditCharacterAccess = false;
};
