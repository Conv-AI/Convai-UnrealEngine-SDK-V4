...
void FOAuthAuthenticationService::OpenBrowserWindow(int32 Port)
{
    ...
    Shell->GetOnWindowClosedEvent().AddLambda([this](const TSharedRef<SWindow, ESPMode::ThreadSafe> &Window)
                                              { CancelLogin(); });
...
#else
    // External browser is used for UE < 5.1, no window to close
#endif
}
            ...
            if (PinnedThis->AuthShell.IsValid())
            {
                ...
                PinnedThis->CloseBrowserTickerHandle = FTSTicker::GetCoreTicker().AddTicker(
                    FTickerDelegate::CreateLambda([WeakSelfPtr](float)
                                                  ...
                                                  auto WindowResult = ConvaiEditor::ErrorHandling::SafeOpenConvaiWindow(true);
...
void FOAuthAuthenticationService::HideLoadingScreen()
{
    if (LoadingWindow.IsValid())
    {
        LoadingWindow.Pin()->RequestDestroyWindow();
        LoadingWindow.Reset();
    }
}