...
UConvaiSubsystem::UConvaiSubsystem()
    ...
    , UserStoppedSpeakingTimestamp(0.0)
    , bUserStoppedSpeakingPending(false)
{
}
...
void UConvaiSubsystem::OnBotStartedSpeaking(const char* attendee_id)
{
    if (!IsValid(CurrentCharacterSession))
    {
        return;
    ...
    // Calculate response latency if we have a pending user stopped speaking event
    if (bUserStoppedSpeakingPending)
    {
        const double CurrentTime = FPlatformTime::Seconds();
        const double LatencyMs = (CurrentTime - UserStoppedSpeakingTimestamp) * 1000.0;
        ResponseLatencies.Add(LatencyMs);
        bUserStoppedSpeakingPending = false;

        // CONVAI_LOG(ConvaiSubsystemLog, Log, TEXT("Response Latency: %.2f ms"), LatencyMs);
...
void UConvaiSubsystem::OnUserStartedSpeaking(const char* attendee_id)
{
    if (!IsValid(CurrentPlayerSession))
    {
        return;
    ...
    // Reset pending flag if user starts speaking again before bot responded
    bUserStoppedSpeakingPending = false;
...
void UConvaiSubsystem::OnUserStoppedSpeaking(const char* attendee_id)
{
    if (!IsValid(CurrentPlayerSession))
    {
        return;
    ...
    UserStoppedSpeakingTimestamp = FPlatformTime::Seconds();
    bUserStoppedSpeakingPending = true;
...
{
    if (ResponseLatencies.Num() == 0)
    {
        return;
    }

    // Create a sorted copy for percentile calculations
    TArray<double> SortedLatencies = ResponseLatencies;
    SortedLatencies.Sort();

    const int32 Count = SortedLatencies.Num();

    // Calculate min, max
    const double Min = SortedLatencies[0];
    const double Max = SortedLatencies[Count - 1];

    // Calculate mean
    double Sum = 0.0;
    for (const double Latency : SortedLatencies)
    {
        Sum += Latency;
    }
    const double Mean = Sum / Count;

    // Calculate quartiles (using nearest rank method)
    auto GetPercentile = [&SortedLatencies, Count](double Percentile) -> double
    {
        if (Count == 1)
        {
            return SortedLatencies[0];
        }
        const double Index = (Percentile / 100.0) * (Count - 1);
        ...
        return SortedLatencies[LowerIndex] * (1.0 - Fraction) + SortedLatencies[UpperIndex] * Fraction;
    };

    const double Q1 = GetPercentile(25.0);
    const double Median = GetPercentile(50.0);
    const double Q3 = GetPercentile(75.0);

    CONVAI_LOG(ConvaiSubsystemLog, Log, TEXT("=== Response Latency Statistics (n=%d) ==="), Count);
    CONVAI_LOG(ConvaiSubsystemLog, Log, TEXT("  Min: %.2f ms"), Min);
    CONVAI_LOG(ConvaiSubsystemLog, Log, TEXT("  Q1 (25%%): %.2f ms"), Q1);
    CONVAI_LOG(ConvaiSubsystemLog, Log, TEXT("  Median: %.2f ms"), Median);
    CONVAI_LOG(ConvaiSubsystemLog, Log, TEXT("  Mean: %.2f ms"), Mean);
    CONVAI_LOG(ConvaiSubsystemLog, Log, TEXT("  Q3 (75%%): %.2f ms"), Q3);
    CONVAI_LOG(ConvaiSubsystemLog, Log, TEXT("  Max: %.2f ms"), Max);
}
