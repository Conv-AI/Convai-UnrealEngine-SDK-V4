...
public class Convai : ModuleRules
{
    ...
    private bool ConfigurePlatform(ReadOnlyTargetRules target, UnrealTargetConfiguration configuration)
    {
        ...
        foreach (var arch in _convaiPlatformInstance.Architectures())
        {
            ...
            if (target.Platform == UnrealTargetPlatform.Win64)
            {
                ...
                if (!Directory.Exists(webRtcArchPath))
                {
                    Log.TraceWarning($"[ConvaiWebRTC] Win64 directory not found: {webRtcArchPath}");
                    continue;
                }
                ...
                foreach (var dllFile in dllFiles)
                {
                    ...
                    // Editor only: a running Editor holds these exact files open (Convai::StartupModule
                    // loads them from this directory), and Windows locks a loaded DLL, so a Game-target
                    // build re-staging the same path fails with "Access to the path is denied" while the
                    // Editor is open. Packaged builds get the DLLs from the two entries below instead.
                ...
                Log.TraceInformation($"[ConvaiWebRTC] Added {libFiles.Length} .lib files and {dllFiles.Length} .dll files for Win64");
            }
            else if (target.Platform == UnrealTargetPlatform.Mac)
            {
                ...
                if (!Directory.Exists(webRtcArchPath))
                {
                    Log.TraceWarning($"[ConvaiWebRTC] Mac directory not found: {webRtcArchPath}");
                    continue;
                }

                // Add all .a files (if any static libs are ever shipped)
                ...
                foreach (var dylibFile in dylibFiles)
                {
                    ...
                    // On Mac, a delay-loaded dylib must be referenced by FULL PATH: UBT's
                    // MacToolChain emits `-weak_library "<path>"` for each PublicDelayLoadDLLs
                    // entry (and adds the matching -rpath). This weak-links the module against
                    // the dylib, which both resolves the convai::* symbols the module calls
                    // directly (e.g. convai::ConvaiClient) and lets the module still load if a
                    // dylib is momentarily absent (Convai::StartupModule() also loads them via
                    // GetDllHandle). Passing a bare name here (the Windows /DELAYLOAD style)
                    // makes ld fail with "library '<name>.dylib' not found". All three dylibs
                    // are passed so the linker can also resolve libconvai_client's sibling
                    // @rpath/... dependencies (liblivekit_ffi, libconvai_http_helper).
                    PublicDelayLoadDLLs.Add(dylibFile);
                ...
                Log.TraceInformation($"[ConvaiWebRTC] Added {aFiles.Length} .a files and {dylibFiles.Length} .dylib files for Mac");
            }
            else if (target.Platform == UnrealTargetPlatform.Linux)
            {
                ...
                if (!Directory.Exists(webRtcArchPath))
                {
                    Log.TraceWarning($"[ConvaiWebRTC] Linux directory not found: {webRtcArchPath}");
                    continue;
                }
                ...
                Log.TraceInformation($"[ConvaiWebRTC] Added {aFiles.Length} .a files and {allSoFiles.Length} .so files for Linux");
            }
            else if (target.Platform == UnrealTargetPlatform.IOS)
            {
                ...
                if (!Directory.Exists(webRtcArchPath))
                {
                    Log.TraceWarning($"[ConvaiWebRTC] iOS directory not found: {webRtcArchPath}");
                    continue;
                }
                ...
                Log.TraceInformation($"[ConvaiWebRTC] Added {aFiles.Length} .a files and {dylibFiles.Length} .dylib files for iOS");
            }
            else if (target.Platform == UnrealTargetPlatform.Android)
            {
                // Check if the architecture directory exists before trying to read files
                if (!Directory.Exists(webRtcArchPath))
                {
                    Log.TraceWarning($"[ConvaiWebRTC] Android architecture directory not found: {webRtcArchPath}");
                    Log.TraceWarning($"[ConvaiWebRTC] Skipping architecture: {arch}");
                    continue;
                }
                ...
                Log.TraceInformation($"[ConvaiWebRTC] Added {aFiles.Length} .a files and {soFiles.Length} .so files for Android {arch}");
            }
        }
    ...
    public Convai(ReadOnlyTargetRules Target) : base(Target)
    {
        ...
        // Every file under Source/Convai/Private/Tests is wrapped in #if WITH_TESTS, which UBT
        // defines as 1 unless the configuration is Test or Shipping, and which is one of the few
        // names UHT's preprocessor understands -- so the UCLASSes in there are scoped rather than
        // dropped from generated code. See FINDINGS F16. The public release script strips the
        // folder as well.
        ...
        PublicDependencyModuleNames.AddRange(new string[] { "Core", "CoreUObject", "Engine", "InputCore", "Json", "JsonUtilities", "AudioMixer", "AudioCaptureCore", "AudioCapture", "Voice", "SignalProcessing", "libOpus", "OpenSSL", "zlib", "SSL", "NavigationSystem", "GameplayTasks", "AIModule", "UMG", "Slate", "SlateCore", "DeveloperSettings" });
...
{
   // Prebuilt macOS dylibs ship under lib/release/macos/<arch>/. Only arm64 is
   // provided today; add "x86_64/" here (and the matching binaries) to support
   // Intel / universal Mac builds.
   public override string LibrariesPath { get { return "macos/"; } }
   public override List<string> Architectures() { return new List<string> { "arm64/" }; }
   public override string LibraryPrefixName { get { return "lib"; } }
   public override string LibraryPostfixName { get { return ".dylib"; } }
}
