...
public:
	...
	/**
	 * Continuously stream audio in regular chunks until all audio is sent.
	 * This simulates a steady streaming scenario where audio arrives at regular intervals.
	 *
	 * @param AudioStreamer			The audio streamer to send chunks to
	 * @param AudioToPlay			The source audio to stream
	 * @param ChunkDurationSeconds	Duration of each chunk in seconds (default: 1.0s)
	 * @param IntervalSeconds		Time between sending each chunk in seconds (default: 1.0s)
	 */
	UFUNCTION(BlueprintCallable, meta = (BlueprintInternalUseOnly = "true", DisplayName = "Stream Audio Continuously", WorldContext = "WorldContextObject"), Category = "Convai|Debug|Tests")
	static UConvaiAudioChunkTestProxy* CreateContinuousAudioStreamProxy(
		UObject* WorldContextObject,
		UConvaiAudioStreamer* AudioStreamer,
		USoundWave* AudioToPlay,
		float ChunkDurationSeconds = 1.0f,
