...
void UConvaiChatBotGetCharsProxy::onHttpRequestComplete(FHttpRequestPtr RequestPtr, FHttpResponsePtr ResponsePtr, bool bWasSuccessful)
{
	...
	{
		CONVAI_LOG(ConvaiBotHttpLog, Warning, TEXT("Failed to receive characters array, Response:%s"), *Response);
		failed();
		return;
	}

	Characters.Reserve(OutArray->Num());
	CharIDs.Reserve(OutArray->Num());
	for (int i = 0; i < OutArray->Num(); i++)
	{
		const TSharedPtr<FJsonObject> Json = (*OutArray)[i]->AsObject();
		if (!Json.IsValid())
		{
			CONVAI_LOG(ConvaiBotHttpLog, Warning, TEXT("Character entry [%d] is not a JSON object, Response:%s"), i, *Response);
			failed();
			return;
		}

		FConvaiAvatarInfo Info;

		Json->TryGetStringField(JSON_FIELD("character_name"), Info.CharacterName);
		Json->TryGetStringField(JSON_FIELD("user_id"),        Info.UserId);
		Json->TryGetStringField(JSON_FIELD("character_id"),   Info.CharacterId);
		Json->TryGetStringField(JSON_FIELD("listing"),        Info.Listing);
		Json->TryGetStringField(JSON_FIELD("voice_type"),     Info.VoiceType);
		Json->TryGetStringField(JSON_FIELD("language_code"),  Info.LanguageCode);
		Json->TryGetStringField(JSON_FIELD("timestamp"),      Info.Timestamp);
		Json->TryGetStringField(JSON_FIELD("model_type"),     Info.ModelType);
		Json->TryGetStringField(JSON_FIELD("description"),    Info.Description);
		Json->TryGetStringField(JSON_FIELD("backstory"),      Info.Backstory);
		Json->TryGetBoolField  (JSON_FIELD("is_narrative_driven"),        Info.bIsNarrativeDriven);
		Json->TryGetStringField(JSON_FIELD("start_narrative_section_id"), Info.StartNarrativeSectionId);
		Json->TryGetBoolField  (JSON_FIELD("edit_character_access"),      Info.bEditCharacterAccess);

		const TArray<TSharedPtr<FJsonValue>>* LanguageCodesArray = nullptr;
		if (Json->TryGetArrayField(JSON_FIELD("language_codes"), LanguageCodesArray))
		{
			for (const TSharedPtr<FJsonValue>& V : *LanguageCodesArray) Info.LanguageCodes.Add(V->AsString());
		}

		const TArray<TSharedPtr<FJsonValue>>* CharacterActionsArray = nullptr;
		if (Json->TryGetArrayField(JSON_FIELD("character_actions"), CharacterActionsArray))
		{
			for (const TSharedPtr<FJsonValue>& V : *CharacterActionsArray) Info.CharacterActions.Add(V->AsString());
		}

		const TArray<TSharedPtr<FJsonValue>>* CharacterEmotionsArray = nullptr;
		if (Json->TryGetArrayField(JSON_FIELD("character_emotions"), CharacterEmotionsArray))
		{
			for (const TSharedPtr<FJsonValue>& V : *CharacterEmotionsArray) Info.CharacterEmotions.Add(V->AsString());
		}

		// model_details can be JSON null in the list response — null-check before reading.
		const TSharedPtr<FJsonObject>* ModelDetailsPtr = nullptr;
		if (Json->TryGetObjectField(JSON_FIELD("model_details"), ModelDetailsPtr) && ModelDetailsPtr && (*ModelDetailsPtr).IsValid())
		{
			const TSharedPtr<FJsonObject>& MD = *ModelDetailsPtr;
			MD->TryGetStringField(JSON_FIELD("modelType"),        Info.ModelDetails.ModelType);
			MD->TryGetStringField(JSON_FIELD("modelLink"),        Info.ModelDetails.ModelLink);
			MD->TryGetStringField(JSON_FIELD("modelPlaceholder"), Info.ModelDetails.ModelPlaceholder);

			const TSharedPtr<FJsonObject>* MHPtr = nullptr;
			if (MD->TryGetObjectField(JSON_FIELD("METAHUMAN"), MHPtr) && MHPtr && (*MHPtr).IsValid())
			{
				const TSharedPtr<FJsonObject>& MH = *MHPtr;
				MH->TryGetStringField(JSON_FIELD("avatar_id"),           Info.ModelDetails.MetaHuman.AvatarId);
				MH->TryGetStringField(JSON_FIELD("avatar_image"),        Info.ModelDetails.MetaHuman.AvatarImage);
				MH->TryGetStringField(JSON_FIELD("avatar_image_square"), Info.ModelDetails.MetaHuman.AvatarImageSquare);
				MH->TryGetStringField(JSON_FIELD("experience_id"),       Info.ModelDetails.MetaHuman.ExperienceId);
				MH->TryGetStringField(JSON_FIELD("bg_image"),            Info.ModelDetails.MetaHuman.BackgroundImage);
			}
		}

		const TSharedPtr<FJsonObject>* GuardrailMetaPtr = nullptr;
		if (Json->TryGetObjectField(JSON_FIELD("guardrail_meta"), GuardrailMetaPtr) && GuardrailMetaPtr && (*GuardrailMetaPtr).IsValid())
		{
			const TSharedPtr<FJsonObject>& GM = *GuardrailMetaPtr;
			GM->TryGetNumberField(JSON_FIELD("limitResponseLevel"), Info.GuardrailMeta.LimitResponseLevel);
			const TArray<TSharedPtr<FJsonValue>>* BlockedWordsArray = nullptr;
			if (GM->TryGetArrayField(JSON_FIELD("blockedWords"), BlockedWordsArray))
			{
				for (const TSharedPtr<FJsonValue>& V : *BlockedWordsArray) Info.GuardrailMeta.BlockedWords.Add(V->AsString());
			}
		}

		const TSharedPtr<FJsonObject>* CharacterTraitsPtr = nullptr;
		if (Json->TryGetObjectField(JSON_FIELD("character_traits"), CharacterTraitsPtr) && CharacterTraitsPtr && (*CharacterTraitsPtr).IsValid())
		{
			const TSharedPtr<FJsonObject>& CT = *CharacterTraitsPtr;
			const TArray<TSharedPtr<FJsonValue>>* CatchPhrasesArray = nullptr;
			if (CT->TryGetArrayField(JSON_FIELD("catch_phrases"), CatchPhrasesArray))
			{
				for (const TSharedPtr<FJsonValue>& V : *CatchPhrasesArray) Info.CharacterTraits.CatchPhrases.Add(V->AsString());
			}
			CT->TryGetStringField(JSON_FIELD("speaking_style"), Info.CharacterTraits.SpeakingStyle);

			const TSharedPtr<FJsonObject>* PTPtr = nullptr;
			if (CT->TryGetObjectField(JSON_FIELD("personality_traits"), PTPtr) && PTPtr && (*PTPtr).IsValid())
			{
				const TSharedPtr<FJsonObject>& PT = *PTPtr;
				PT->TryGetNumberField(JSON_FIELD("openness"),       Info.CharacterTraits.PersonalityTraits.Openness);
				PT->TryGetNumberField(JSON_FIELD("sensitivity"),    Info.CharacterTraits.PersonalityTraits.Sensitivity);
				PT->TryGetNumberField(JSON_FIELD("extraversion"),   Info.CharacterTraits.PersonalityTraits.Extraversion);
				PT->TryGetNumberField(JSON_FIELD("agreeableness"),  Info.CharacterTraits.PersonalityTraits.Agreeableness);
				PT->TryGetNumberField(JSON_FIELD("meticulousness"), Info.CharacterTraits.PersonalityTraits.Meticulousness);
			}
		}

		// Verbosity / moderation flags / allowed filters / temperature live under
		// personalised_prompt_config in the list response (different shape from
		// /character/get where they were top-level).
		const TSharedPtr<FJsonObject>* PPCPtr = nullptr;
		if (Json->TryGetObjectField(JSON_FIELD("personalised_prompt_config"), PPCPtr) && PPCPtr && (*PPCPtr).IsValid())
		{
			const TSharedPtr<FJsonObject>& PPC = *PPCPtr;
			PPC->TryGetNumberField(JSON_FIELD("VERBOSITY"),          Info.Verbosity);
			PPC->TryGetBoolField  (JSON_FIELD("MODERATION_ENABLED"), Info.bModerationEnabled);
			PPC->TryGetNumberField(JSON_FIELD("TEMPERATURE"),        Info.Temperature);
			const TArray<TSharedPtr<FJsonValue>>* AllowedFiltersArray = nullptr;
			if (PPC->TryGetArrayField(JSON_FIELD("ALLOWED_MODERATION_FILTERS"), AllowedFiltersArray))
			{
				for (const TSharedPtr<FJsonValue>& V : *AllowedFiltersArray) Info.AllowedModerationFilters.Add(V->AsString());
			}
		}

		const TSharedPtr<FJsonObject>* MemorySettingsPtr = nullptr;
		if (Json->TryGetObjectField(JSON_FIELD("memory_settings"), MemorySettingsPtr) && MemorySettingsPtr && (*MemorySettingsPtr).IsValid())
		{
			(*MemorySettingsPtr)->TryGetBoolField(JSON_FIELD("enabled"), Info.MemorySettings.bEnabled);
		}

		const TSharedPtr<FJsonObject>* SpeakingStylePtr = nullptr;
		if (Json->TryGetObjectField(JSON_FIELD("speaking_style"), SpeakingStylePtr) && SpeakingStylePtr && (*SpeakingStylePtr).IsValid())
		{
			const TSharedPtr<FJsonObject>& SS = *SpeakingStylePtr;
			SS->TryGetStringField(JSON_FIELD("description"),      Info.SpeakingStyle.Description);
			SS->TryGetStringField(JSON_FIELD("sample_dialogues"), Info.SpeakingStyle.SampleDialogues);
		}

		const TSharedPtr<FJsonObject>* EmbodimentDataPtr = nullptr;
		if (Json->TryGetObjectField(JSON_FIELD("embodiment_data"), EmbodimentDataPtr) && EmbodimentDataPtr && (*EmbodimentDataPtr).IsValid())
		{
			const TSharedPtr<FJsonObject>& ED = *EmbodimentDataPtr;
			ED->TryGetStringField(JSON_FIELD("looks_description"),   Info.EmbodimentData.LooksDescription);
			ED->TryGetStringField(JSON_FIELD("clothes_description"), Info.EmbodimentData.ClothesDescription);
		}

		// Mirror character_id into the legacy CharIDs pin so existing BP graphs
		// wired to the old "Array of String" output don't break.
		CharIDs.Add(Info.CharacterId);
		Characters.Add(MoveTemp(Info));
	}
...
{
	OnFailure.Broadcast(CharIDs, Characters);
	finish();
}

void UConvaiChatBotGetCharsProxy::success()
{
	OnSuccess.Broadcast(CharIDs, Characters);
	finish();
}
...
void UConvaiDownloadImageProxy::Activate()
{
	...
	if (URL.EndsWith("/"))
	{
		URL.RemoveAt(URL.Len() - 1, 1, CONVAI_ALLOW_SHRINKING_NO);
	}
...
bool UConvaiGetAvailableVoicesProxy::ParseAllVoiceData(const FString &JsonString, TMap<FString, FVoiceLanguageStruct> &FilteredVoices)
{
	...
	if (FJsonSerializer::Deserialize(Reader, JsonObject) && JsonObject.IsValid())
	{
		...
		{
			// FJsonObject::Values key changed from FString (5.7) to TSharedString (5.8).
			// *VoiceTypeEntry.Key is const TCHAR* in both; FString(const TCHAR*) is portable.
