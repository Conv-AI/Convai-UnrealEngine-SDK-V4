...
public:
	...
	*/
	...
	/**
	 * EnvironmentData.Actions plus any enabled built-in actions (skipping
	 * built-ins the designer shadowed with a same-named entry). This is the
...
private:
	...
	/** Dispatch fallback for built-in actions, tried only after the owner actor
	 *  and this component had their FindFunction chance — a user handler with
	 *  the action's name always wins. Gated on ConnectedBuiltInNames (what the
	 *  server was actually told at connect), NOT the live toggles. Game thread
	 *  (via TriggerNamedBlueprintAction). */
	...
	/** Reconciles persistent, Never-response usage hints with the built-ins the
	 *  current connection actually advertised. Called once at session start. */
	void SyncBuiltInActionContextFacts();
