...
namespace
{
	...
	bool TryMatchWhitespaceCompactedActionPrefix(
		const FString& FilledIn,
		const FString& CanonicalName,
		int32& OutConsumedLength)
	{
		OutConsumedLength = INDEX_NONE;
		...
		while (CanonicalIndex < CanonicalName.Len())
		{
			if (FChar::IsWhitespace(CanonicalName[CanonicalIndex]))
			{
				++CanonicalIndex;
				continue;
			}
			while (FilledIndex < FilledIn.Len() &&
				FChar::IsWhitespace(FilledIn[FilledIndex]))
			{
				++FilledIndex;
			}
			if (FilledIndex >= FilledIn.Len() ||
				FChar::ToLower(FilledIn[FilledIndex]) !=
					FChar::ToLower(CanonicalName[CanonicalIndex]))
			{
				return false;
			}
			++FilledIndex;
		...
		if (!HasCanonicalActionBoundary(FilledIn, FilledIndex))
		{
			return false;
		}
		OutConsumedLength = FilledIndex;
		return true;
	}
...
const FConvaiAction* UConvaiActions::FindActionTemplate(const FString& FilledIn, const TArray<FConvaiAction>& Templates)
{
	...
	// Some model responses compact whitespace inside an otherwise exact advertised
	// action name (for example `MostrarRutina Piel Sensible` for `Mostrar rutina`).
	// Accept that compatibility shape only at a real action/value boundary. If two
	// templates collapse to the same prefix length, leave the response unresolved
	// instead of guessing between them.
	const FConvaiAction* LongestCompactedPrefix = nullptr;
	int32 LongestCompactedPrefixLength = INDEX_NONE;
	bool bCompactedPrefixIsAmbiguous = false;
	for (const FConvaiAction& T : Templates)
	{
		int32 ConsumedLength = INDEX_NONE;
		if (!TryMatchWhitespaceCompactedActionPrefix(
			TrimmedFilledIn, T.Name, ConsumedLength))
		{
			continue;
		}
		if (ConsumedLength > LongestCompactedPrefixLength)
		{
			LongestCompactedPrefix = &T;
			LongestCompactedPrefixLength = ConsumedLength;
			bCompactedPrefixIsAmbiguous = false;
		}
		else if (ConsumedLength == LongestCompactedPrefixLength)
		{
			bCompactedPrefixIsAmbiguous = true;
		}
	}
	if (LongestCompactedPrefix != nullptr)
	{
		return bCompactedPrefixIsAmbiguous ? nullptr : LongestCompactedPrefix;
	}
...
FString UConvaiActions::StripActionPrefix(const FString& FilledIn, const FString& CanonicalName)
{
	...
	{
		int32 CompactedPrefixLength = INDEX_NONE;
		if (!TryMatchWhitespaceCompactedActionPrefix(
			Trimmed, CanonicalName, CompactedPrefixLength))
		{
			return Trimmed;
		}
		return Trimmed.RightChop(CompactedPrefixLength).TrimStartAndEnd();
	}
