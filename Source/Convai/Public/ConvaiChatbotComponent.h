...
public:
	...
	*/
	 ...
	 *
	 *    Only sent to the server when bEnableActions is true.
	 */
	/** Action affordance contract data — populated at edit time, mutated at runtime via the
	 *  granular Add/Remove/Clear methods. The legacy BP-visible `Environment` UObject pointer
	 *  below is the deprecated migration shim; new code should use this field via the methods
	 *  on the chatbot, never read this directly. */
	...
	/**
	 *    When true, the Environment struct is serialized as `action_config` and sent at /connect time.
	 *    Leave false for purely conversational characters that don't perform physical actions.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Convai|Actions", Replicated)
	 ...
	 *    an attention object, an interesting prop, or anything else. Per-client (not replicated).
	 */
	UPROPERTY(BlueprintReadWrite, Category = "Convai|Animation")
	...
	/**
	 * Delay in seconds before a staged context update is sent. Additional
	 * updates arriving within the window reset the timer and are aggregated
	 * into a single flush.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Convai|DynamicContext", meta = (ClampMin = "0.0", UIMin = "0.0"))
	float ContextAggregationDelay = 0.5f;
	 ...
	 *
	 * Has no effect when bEnableActions is false on this chatbot — the server only
	 * resolves attention when action_config was sent at /connect.
	 */
	UFUNCTION(BlueprintCallable, Category = "Convai|Actions",
		meta = (AdvancedDisplay = "bFlushImmediately",
				ToolTip = "Set attention object + optional event text. Has no effect when bEnableActions is false.\n\nbFlushImmediately: send right away instead of batching. Use sparingly."))
	...
	/**
	 * Marks the current action as completed and handles post-execution logic. Optionally
	 * fires a dynamic-context event in the same call so the bot is told what just
	 * happened (e.g. "Picked up the cube"). Equivalent to calling AddContextEvent
	 * separately, but lets handlers report completion + outcome in one step.
	 *
	 * @param IsSuccessful   true → next action in the queue is processed; false → the current action is retried.
	 * @param Delay          Seconds to wait before the next StartFirstAction call.
	 * @param EventText      Optional event text describing what happened. Empty (default)
	 *                       means "no event" — nothing is added to the dynamic context.
	 * @param ShouldRespond  Whether the bot should respond to the event. Auto = let the
	 *                       LLM decide; Always = force a response; Never = quiet update.
	 *                       Ignored when EventText is empty.
	 */
	UFUNCTION(BlueprintCallable, Category = "Convai|Actions",
		meta = (AdvancedDisplay = "EventText,ShouldRespond"))
	void HandleActionCompletion(bool IsSuccessful = true, float Delay = 0.0,
		FString EventText = TEXT(""),
		EC_RunLLMOption ShouldRespond = EC_RunLLMOption::Auto);
...
private:
	...
	/** Reset the flush timer to now + ContextAggregationDelay. */
...
private:
	...
	{
		return bEnableActions ? EnvironmentData.ToActionConfigJson() : FString();
	}
...
private:
	...
	/** Grace window after OnFinishedTalking during which audio chunks are
	 *  still passed through. Short stutters and intentional micro-gaps stay
	 *  aligned with lipsync. Past this window, audio is dropped — any lipsync
	 *  frames for it don't exist (frames only flow during talking intervals),
	 *  so feeding it would desync the character. */
