...
public:
	...
	/**
	 * Whether releasing the button should tell the server the turn is over.
	 *
	 * False only while the server has already ended and answered this hold's turn, where a
	 * second stop commits an empty buffer. Everything else answers true, including a hold the
	 * server never bracketed at all: a dropped or never-sent user-started-speaking must not
	 * strand a turn, and PushToTalkStopSecs leaves the release as the only thing that ends one.
	 */
	bool ShouldEndTurnOnRelease() const { return !bServerEndedTurnThisHold; }
	...
	/** Whether the server should keep speech-to-text muted: when nothing is
	 *  streaming, or under push-to-talk while the button is up. */
	static bool ShouldMuteStt(bool bStreaming, bool bPushToTalk, bool bMuted)
	{
		return !bStreaming || (bPushToTalk && bMuted);
	}
...
public:
	...
	/** What the microphone tap is actually receiving. Zeros when it is not
	 *  registered — which is itself the answer when the mic stream looks wrong. */
	void GetMicTapFormat(int32& OutChannels, int32& OutSampleRate, int64& OutChunksRouted) const;
...
private:
	...
	/**
	 * Microphone capture, taken from the submix rather than from the output
	 * recorder.
	 *
	 * The recorder is stopped for the whole of each cycle's read, convert and
	 * send, and the mixer drops what it renders in that window. Measured, that
	 * cost the mic stream 1-2.5% of its samples against the far-end stream over
	 * the same window - the two timelines separating at 10-25 ms per second,
	 * which no canceller can track. The listener is handed every rendered
	 * buffer, so the two streams stay on one clock.
	 */
	friend class FConvaiMicSubmixTap;
#if WITH_TESTS
	friend class FConvaiMicTapBacklogTest;
#endif
	void AppendMicTapSamples(const float* Samples, int32 NumSamples, int32 NumChannels, int32 SampleRate);
	void DrainMicTap();
	bool EnsureMicTapRegistered();
	void UnregisterMicTap();
	/** Shared tail of both capture paths: recording buffer, processing, send. */
	void RouteCapturedMicChunk(const int16* Samples, int32 NumSamples);

	// Latched once per capture start from the MicCaptureTap custom param.
	bool bUseMicListenerTap = false;
	TSharedPtr<FConvaiMicSubmixTap, ESPMode::ThreadSafe> MicTap;

	// Render thread writes, audio thread drains. Raw interleaved source format;
	// conversion happens on the drain so the callback stays cheap.
	mutable FCriticalSection MicTapBufferLock;
	TArray<float> MicTapBuffer;
	int32 MicTapChannels = 0;
	int32 MicTapSampleRate = 0;
	bool bWarnedMicTapOverflow = false;
	// Diagnostic only: how many 10 ms chunks the tap has routed.
	int64 MicTapChunksRouted = 0;
	/** Printed on an overflow, and the pair is what tells the causes apart:
	 *  engine frames since the drain last ran, and dispatches queued since. */
	uint64 MicTapDrainFrame = 0;
	int32 MicTapDispatchesSinceDrain = 0;

	// Audio thread only.
	TUniquePtr<FConvaiPcmReblocker> MicReblocker;
	...
	/** Follows the closed speaking bracket rather than the raw server stop, so a release inside
	 *  the stop grace still ends the turn: a stray stop costs one advisory, a suppressed one
	 *  costs the whole reply. */
	bool bServerEndedTurnThisHold = false;
	...
	virtual void OnSessionReady() override { ResyncStt(); }
	virtual void OnReadyTimedOut() override { ResyncStt(); }
	virtual void OnAttendeeConnected(FString AttendeeId) override;

	/** A new server session starts with its own speech-to-text state: tell it
	 *  the one this player is actually in. */
	void ResyncStt();
