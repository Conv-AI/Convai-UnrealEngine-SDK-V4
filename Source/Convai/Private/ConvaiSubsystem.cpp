...
uint32 FConvaiConnectionThread::Run()
{
    if (ConnectionParams.Client)
    {
        ...
        char ActionConfigBuffer[METADATA_BUFFER_SIZE];
        ...
        const int32 ActionConfigLen = ConnectionParams.ActionConfigJson.IsEmpty()
            ? 0
            : SafeConvertToUTF8(ActionConfigBuffer, METADATA_BUFFER_SIZE, *ConnectionParams.ActionConfigJson);

        // Validate all conversions succeeded (ActionConfigLen of 0 is valid — means "not opted in")
        if (StreamURLLen < 0 || AuthKeyHeaderLen < 0 || AuthKeyValueLen < 0 || CharIDLen < 0 || ConnectionTypeLen < 0 || LLMProviderLen < 0 || BlendshapeProviderLen < 0 || BlendshapeFormatLen < 0 || EmotionProviderLen < 0 || EndUserIDLen < 0 || EndUserMetadataLen < 0 || ActionConfigLen < 0)
        {
        ...
        {
            CONVAI_LOG(ConvaiSubsystemLog, Log, TEXT("ActionConfig: %s"), *ConnectionParams.ActionConfigJson);
        }
        ...
        ConvaiConnectionConfig.action_config = ActionConfigLen > 0 ? ActionConfigBuffer : nullptr;
        ...
        char InvocationSourceBuffer[BUFFER_SIZE];
        char InvocationClientVersionBuffer[BUFFER_SIZE];
        char InvocationExtraMetadataBuffer[METADATA_BUFFER_SIZE];

        const int32 InvocationSourceLen = SafeConvertToUTF8(InvocationSourceBuffer, BUFFER_SIZE, TEXT("ue_sdk"));
        const int32 InvocationClientVersionLen = SafeConvertToUTF8(InvocationClientVersionBuffer, BUFFER_SIZE, *UConvaiUtils::GetClientVersion());
        const int32 InvocationExtraMetadataLen = SafeConvertToUTF8(InvocationExtraMetadataBuffer, METADATA_BUFFER_SIZE, *ExtraMetadataString);

        convai::ConvaiInvocationMetadata InvocationMetadata;
        if (InvocationSourceLen >= 0 && InvocationClientVersionLen >= 0 && InvocationExtraMetadataLen >= 0)
        {
            InvocationMetadata.source = InvocationSourceBuffer;
            InvocationMetadata.client_version = InvocationClientVersionBuffer;
            InvocationMetadata.extra_metadata = InvocationExtraMetadataBuffer;
            ConvaiConnectionConfig.invocation_metadata = &InvocationMetadata;

            CONVAI_LOG(ConvaiSubsystemLog, Log, TEXT("InvocationMetadata - ClientVersion: %s | ExtraMetadata: %s"), UTF8_TO_TCHAR(InvocationClientVersionBuffer), *ExtraMetadataString);
        }
        else
        {
            CONVAI_LOG(ConvaiSubsystemLog, Warning, TEXT("Failed to convert invocation metadata to UTF8, connecting without it"));
        }
...
void UConvaiSubsystem::RegisterObjectComponent(UConvaiObjectComponent* ObjectComponent)
{
    ...
    // Auto-disambiguate name on collision. First registration wins its bare name;
    // subsequent conflicts get " 2", " 3", ... suffix (plain space + number, no
    // underscore or zero-padding so TTS reads "Door two" instead of "Door underscore
    // zero two"). Already-broadcast context-state keys on the original component
    // stay stable. Case-insensitive collision check so "Door"/"door" don't both
    // reach the LLM.
    const FString OriginalName = ObjectComponent->ObjectEntry.Name;
    FString UniqueName = OriginalName;
    int32 SuffixN = 2;

    auto HasCollision = [&](const FString& Candidate) -> bool
    {
        for (UConvaiObjectComponent* Other : RegisteredObjectComponents)
        {
            if (!IsValid(Other)) continue;
            if (Other->ObjectEntry.Name.Equals(Candidate, ESearchCase::IgnoreCase))
            {
                return true;
            }
        }
    ...
    while (HasCollision(UniqueName))
    {
        UniqueName = FString::Printf(TEXT("%s %d"), *OriginalName, SuffixN++);
    }

    if (!UniqueName.Equals(OriginalName, ESearchCase::CaseSensitive))
    {
        CONVAI_LOG(ConvaiSubsystemLog, Warning,
            TEXT("ConvaiObjectComponent name '%s' already taken; auto-renamed to '%s' for component on actor %s."),
            *OriginalName, *UniqueName,
            ObjectComponent->GetOwner() ? *ObjectComponent->GetOwner()->GetName() : TEXT("<no owner>"));
        ObjectComponent->ObjectEntry.Name = UniqueName;
    }

    RegisteredObjectComponents.Add(ObjectComponent);
    // First registration -> start the shared clock.
    RestartObjectPollClock();
}
...
void UConvaiSubsystem::UnregisterObjectComponent(UConvaiObjectComponent* ObjectComponent)
{
    if (RegisteredObjectComponents.Contains(ObjectComponent))
    {
        ...
        if (RegisteredObjectComponents.Num() == 0)
        {
            StopObjectPollClock();
        }
    }
...
float UConvaiSubsystem::ComputeEffectiveObjectPollInterval()
{
    // Plain user-configured tick rate. The chatbot's own debounce window
    // already coalesces same-tick context updates, so the poll clock no
    // longer enforces a floor relative to it — designers get exactly the
    // cadence they ask for.
    return UConvaiUtils::GetObjectPollIntervalSeconds();
}

void UConvaiSubsystem::RestartObjectPollClock()
{
    StopObjectPollClock();
    if (RegisteredObjectComponents.Num() == 0)
    {
    ...
    ObjectPollTickerHandle = FTSTicker::GetCoreTicker().AddTicker(
        FTickerDelegate::CreateUObject(this, &UConvaiSubsystem::TickObjectPollClock),
        Interval);
}

void UConvaiSubsystem::StopObjectPollClock()
{
...
bool UConvaiSubsystem::TickObjectPollClock(float /*DeltaTime*/)
{
                    ...
                    case EC_ServerPacketType::ActionResponse:
                        {
                            ...
                            for (const TSharedPtr<FJsonValue>& Value : *ActionsArray)
                            {
                                ...
                                if (Chatbot != nullptr)
                                {
                                    Tpl = UConvaiActions::FindActionTemplate(RawName, Chatbot->EnvironmentData.Actions);
                                }
                                ...
                                if (Tpl)
                                {
                                    ...
                                    if (Tpl->Parameters.Num() > 0)
                                    {
                                        ...
                                        for (int32 i = 0; i < FMath::Min(Tpl->Parameters.Num(), Values.Num()); ++i)
                                        {
                                            ...
                                            const FString CleanValue = UConvaiActions::StripParamNameMimicry(Values[i], Decl.Name);
                                            ...
                                            if (Decl.Type == EConvaiActionParamType::Enum && !Decl.EnumType)
                                            {
                                                CONVAI_LOG(ConvaiSubsystemLog, Warning,
