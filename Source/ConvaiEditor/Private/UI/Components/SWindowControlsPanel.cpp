...
#if CONVAI_MCP_SUPPORTED
    // AI coding / MCP project setup (UE 5.8+ only — the engine MCP plugins don't exist before 5.8).
    Items.Add({LOCTEXT("OpenSceneAutoTagger", "Scene Auto Tagger"),
               FSimpleDelegate::CreateLambda([this]()
                                             { OnOpenSceneAutoTaggerClicked(); })});

    Items.Add({LOCTEXT("SetupMcp", "Set Up AI Coding (MCP)"),
               FSimpleDelegate::CreateLambda([this]()
                                             { OnSetupMcpClicked(); })});
#endif
...
void SWindowControlsPanel::OnSetupMcpClicked()
{
#if CONVAI_MCP_SUPPORTED
	SConvaiMcpSetupDialog::RunSetupFlow();
#endif
}

void SWindowControlsPanel::OnOpenSceneAutoTaggerClicked()
{
#if CONVAI_MCP_SUPPORTED
	// Load by name to avoid a ConvaiEditor -> AutoTagger module dependency cycle.
    static const FName SceneAutoTaggerTabName(TEXT("ConvaiSceneAutoTagger"));
    if (!FModuleManager::Get().LoadModulePtr<IModuleInterface>(TEXT("ConvaiSceneTaggingEditor")))
    {
        FNotificationInfo Info(LOCTEXT("SceneAutoTaggerUnavailable", "Scene Auto Tagger could not be loaded."));
        Info.ExpireDuration = 5.0f;
        FSlateNotificationManager::Get().AddNotification(Info);
        return;
    }
    FGlobalTabmanager::Get()->TryInvokeTab(SceneAutoTaggerTabName);
#endif
}
