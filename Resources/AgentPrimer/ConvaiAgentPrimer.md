...
- Never wire **Start Session** to a **Disconnected** event to reconnect — set **Reconnect Attempts** (Project Settings ▸ Plugins ▸ Convai ▸ Connection) and the plugin reconnects by itself. Show reconnects from **On Server Connection State Changed** (Reconnecting, then Connected or Disconnected); **Idle Timeout Behavior** in the same settings decides what an idle close does.
