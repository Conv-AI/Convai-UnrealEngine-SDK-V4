...
namespace
{
	int32 HeldLaneRunLLMRank(EC_RunLLMOption Option)
	{
		switch (Option)
		{
...
void FConvaiPendingContextBatch::DowngradeToNever()
{
	for (TMap<FString, EC_RunLLMOption>* Respond : { &StateRespond, &DeclarativeRespond, &EventRespond, &EphemeralEventRespond })
	{
		for (TPair<FString, EC_RunLLMOption>& Pair : *Respond)
		{
			Pair.Value = EC_RunLLMOption::Never;
		}
	}
	WatchPromotedStateKeys.Empty();
	PendingAttentionRunLLM = EC_RunLLMOption::Never;
...
{
	if (HeldLaneRunLLMRank(In) > HeldLaneRunLLMRank(AggregateRunLLM))
	{
		AggregateRunLLM = In;
	}
}
	...
	bool bPreserveTransition)
{
	...
	if (HeldLaneRunLLMRank(ShouldRespond) > HeldLaneRunLLMRank(Response))
	{
...
void FConvaiPendingContextBatch::StageAttention(const FConvaiObjectEntry& Object, const FString& Text, EC_RunLLMOption ShouldRespond)
{
	...
	if (HeldLaneRunLLMRank(ShouldRespond) > HeldLaneRunLLMRank(PendingAttentionRunLLM)
...
namespace
{
	...
	{
		return HeldLaneRunLLMRank(A) >= HeldLaneRunLLMRank(B) ? A : B;
	}
}
...
void FConvaiHeldContextLane::DowngradeToNever()
{
	for (TMap<FString, EC_RunLLMOption>* Respond : { &StateRespond, &DeclarativeRespond })
	{
		for (TPair<FString, EC_RunLLMOption>& Pair : *Respond)
		{
			Pair.Value = EC_RunLLMOption::Never;
	...
	for (FHeldEvent& Event : Events)
	{
		Event.ShouldRespond = EC_RunLLMOption::Never;
	}
	WatchPromotedStateKeys.Empty();
	AttentionRespond = EC_RunLLMOption::Never;
}
