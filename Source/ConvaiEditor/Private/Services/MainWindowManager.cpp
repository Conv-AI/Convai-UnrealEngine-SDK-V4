...
void FMainWindowManager::CloseMainWindow()
{
    ...
    if (Window.IsValid())
    {
        ...
        auto NavSvcResult = FConvaiDIContainerManager::Get().Resolve<INavigationService>();
        if (NavSvcResult.IsSuccess())
        {
            NavSvcResult.GetValue()->ResetWindowState();
        }
...
{
    FConvaiDIContainerManager::Get().Resolve<INavigationService>().LogOnFailure(LogConvaiEditor, TEXT("Failed to resolve NavigationService")).Tap([&Window](TSharedPtr<INavigationService> NavSvc)
                                                                                                                                                  {
...
bool FMainWindowManager::ShouldShowWelcomeFlow() const
{
    ...
    if (WelcomeSvcResult.IsSuccess())
    {
        ...
        return !WelcomeSvc->HasCompletedWelcome() || !WelcomeSvc->HasValidApiKey();
    }
