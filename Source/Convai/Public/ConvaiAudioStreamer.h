...
#if ENGINE_MAJOR_VERSION >= 5 && ENGINE_MINOR_VERSION >= 4
	// Asynchronously locates the FSoundSource for the active sound and caches it along with
	// the current NumFramesPlayed baseline so GetAudioPlaybackTime() can read progress directly.
	// If the source is not yet assigned (sound still initializing), retries once after 0.1s.
	void CacheSoundSource(bool bIsRetry = false);
#endif
...
#if ENGINE_MAJOR_VERSION >= 5 && ENGINE_MINOR_VERSION >= 4
// Cached FSoundSource for direct GetNumFramesPlayed() access on the game thread.
// Re-cached at the start of each new talking session via CacheSoundSource().
FSoundSource* CachedSoundSource;

// NumFramesPlayed value recorded when CacheSoundSource() ran, used as the session baseline.
int64 FramesPlayedAtSessionStart;
#endif
