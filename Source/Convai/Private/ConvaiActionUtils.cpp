	...
	const UEnum* EnumType)
{
	...
	// Resolve against env objects then characters.
	...
	// Enum byte resolution: when the template declared an EnumType, look up the
	// LLM's value against the enum's display names (which is what we serialized in
	// the wire format). On match, ByteValue holds the underlying enum value so
	// handlers can Byte-to-Enum<YourType> in BP.
	if (DeclaredType == EConvaiActionParamType::Enum && EnumType != nullptr)
	{
		const int32 NumValues = EnumType->NumEnums() - (EnumType->ContainsExistingMax() ? 1 : 0);
		for (int32 i = 0; i < NumValues; ++i)
		{
			FString Label = EnumType->GetDisplayNameTextByIndex(i).ToString();
			if (Label.IsEmpty()) { Label = EnumType->GetNameStringByIndex(i); }
			if (Label.Equals(Clean, ESearchCase::IgnoreCase))
			{
				P.ByteValue = static_cast<uint8>(EnumType->GetValueByIndex(i));
				break;
			}
		}
	}
...
FConvaiResultParam UConvaiActions::GetFirstParam(const FConvaiResultAction& Action)
{
	...
	{
		return Pair.Value; // TMap iteration follows insertion order in UE.
	}
