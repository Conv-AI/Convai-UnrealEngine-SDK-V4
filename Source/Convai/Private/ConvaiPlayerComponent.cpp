...
void UConvaiPlayerComponent::TickGazeAttention(float DeltaTime)
{
	...
	if (HitActor != CurrentlyGazedActor.Get() || HitPrim != CurrentlyGazedPrimitive.Get() || bGazeTargetGone)
	{
		...
		if (HitActor)
		{
			...
			SpawnHighlightFor(HitActor, HighlightScope);
...
void UConvaiPlayerComponent::SpawnHighlightFor(AActor* TargetActor, USceneComponent* TargetComponent)
{
	if (!TargetActor)
	{
		return;
	...
	AConvaiGazeHighlightActor* Highlight = World->SpawnActor<AConvaiGazeHighlightActor>(
		Class, TargetActor->GetActorLocation(), FRotator::ZeroRotator, Spawn);
	if (!Highlight)
	{
		return;
	}
	...
	// whole actor's meshes (door's "Handle" silhouette, not the whole door).
	Highlight->SetTarget(TargetActor, TargetComponent);
...
void UConvaiPlayerComponent::PromoteToAttention(AActor* NewActor, UPrimitiveComponent* NewPrimitive)
{
	...
	for (UConvaiObjectComponent* Obj : Matches)
	{
		...
		Obj->NotifyGazeAttentionBegin(this, GazeAttentionText, GazeShouldRespond);
