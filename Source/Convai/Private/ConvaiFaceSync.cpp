...
TArray<float> UConvaiFaceSyncComponent::ConvaiGetFacialData() const
{
	...
	FScopeLock Lock(&BlendShapesMapLock);
...
UConvaiFaceSyncComponent::UConvaiFaceSyncComponent()
{
	...
	MainSequenceBuffer.AnimationFrames.Reserve(1024);
}
...
void UConvaiFaceSyncComponent::BeginPlay()
{
	...
	bSimulateFreeze = UConvaiUtils::IsFaceSyncSimulateFreeze();
	CONVAI_LOG(ConvaiFaceSyncLog, Log, TEXT("LipSyncTimeOffset: %fs, SimulateFreeze: %s"), LipSyncTimeOffset, bSimulateFreeze ? TEXT("true") : TEXT("false"));
}
...
bool UConvaiFaceSyncComponent::CalculateFrameSelection(FFrameSelectionResult& OutResult)
{
	...
	if (bSimulateFreeze && CurrentSequenceTimePassed >= 1.0f && CurrentSequenceTimePassed <= 4.0f)
	{
		CurrentSequenceTimePassed = 99999.0f;
	}


	if (CurrentSequenceTimePassed > MainSequenceBuffer.Duration)
	{
		const double StarvationGap = CurrentSequenceTimePassed - MainSequenceBuffer.Duration;
		if (!bWaitingForFaceFrames)
		{
			CONVAI_LOG(ConvaiFaceSyncLog, Verbose,
				TEXT("Lip sync waiting for face frames (audio %.3fs, buffered %.3fs)"),
				CurrentSequenceTimePassed, MainSequenceBuffer.Duration);
			bWaitingForFaceFrames = true;
		}

		if (!bStarvationFallbackApplied
			&& StarvationGap > UConvaiUtils::GetLipSyncStarvationFallback())
		{
			CONVAI_LOG(ConvaiFaceSyncLog, Warning,
				TEXT("Lip sync starvation exceeded fallback window by %.3fs"), StarvationGap);
			SetCurrentFrameToZero();
			bStarvationFallbackApplied = true;
		}
	...
	{
		CONVAI_LOG(ConvaiFaceSyncLog, Verbose,
			TEXT("Lip sync recovered from face-frame starvation at audio time %.3fs"),
			CurrentSequenceTimePassed);
		bWaitingForFaceFrames = false;
		bStarvationFallbackApplied = false;
	}
	...
	FActorComponentTickFunction* ThisTickFunction)
{
	...
	const double FrameSelStart = FPlatformTime::Seconds();
#endif
...
#if ConvaiDebugMode
	const double FrameSelMs = (FPlatformTime::Seconds() - FrameSelStart) * 1000.0;
	const double InterpStart = FPlatformTime::Seconds();
#endif

	// Apply interpolation if enabled, otherwise just use the end frame for better performance
	if (bEnableInterpolation)
	{
		TMap<FName, float> Interpolated = InterpolateFrames(FrameSelection.StartFrame, FrameSelection.EndFrame, FrameSelection.Alpha);
		FScopeLock Lock(&BlendShapesMapLock);
		CurrentBlendShapesMap = MoveTemp(Interpolated);
	}
	else
	{
		FScopeLock Lock(&BlendShapesMapLock);
...
#if ConvaiDebugMode
	const double InterpMs = (FPlatformTime::Seconds() - InterpStart) * 1000.0;
	const double PostProcStart = FPlatformTime::Seconds();
#endif

	ApplyPostProcessing();

#if ConvaiDebugMode
	const double PostProcMs = (FPlatformTime::Seconds() - PostProcStart) * 1000.0;
	const double DelegateStart = FPlatformTime::Seconds();
#endif
...
#if ConvaiDebugMode
	const double DelegateMs = (FPlatformTime::Seconds() - DelegateStart) * 1000.0;
	const double TickMs = (FPlatformTime::Seconds() - TickStart) * 1000.0;

	FaceSyncStats.SampleCount++;
	...
	AccumStat(FaceSyncStats.Tick, TickMs);
	AccumStat(FaceSyncStats.FrameSel, FrameSelMs);
	AccumStat(FaceSyncStats.Interp, InterpMs);
	AccumStat(FaceSyncStats.PostProc, PostProcMs);
	AccumStat(FaceSyncStats.Delegate, DelegateMs);

	if (FaceSyncStats.SampleCount >= 60)
	{
		const int32 N = FaceSyncStats.SampleCount;
		int32 NumEntries;
		{
			FScopeLock Lock(&BlendShapesMapLock);
			NumEntries = CurrentBlendShapesMap.Num();
		}
		auto LogStat = [N](const FConvaiTimingStat& S) -> FString
		{
			return FString::Printf(TEXT("mean=%.3f min=%.3f max=%.3f"), S.Total / N, S.Min, S.Max);
		};
		CONVAI_LOG(ConvaiFaceSyncLog, Log,
			TEXT("[FaceSync %d ticks] Tick(%s) | FrameSel(%s) | Interp(%s) | PostProc(%s) | Delegate(%s) | Entries=%d"),
			N,
			*LogStat(FaceSyncStats.Tick),
			*LogStat(FaceSyncStats.FrameSel),
			*LogStat(FaceSyncStats.Interp),
			*LogStat(FaceSyncStats.PostProc),
			*LogStat(FaceSyncStats.Delegate),
			NumEntries);
		FaceSyncStats.Reset();
	}
#endif
...
{
	{
		...
		if (GetLipSyncMode() == EC_LipSyncMode::VisemeBased)
		{
			...
			if (sil != nullptr && *sil < 0)
			{
				...
				MainSequenceBuffer.AnimationFrames.Reset();
...
bool UConvaiFaceSyncComponent::HasPlayableFrames() const
{
	// Paused = not currently producing frames
	if (bIsPaused)
	{
		return false;
	}

	// No sequence data buffered at all
	if (MainSequenceBuffer.AnimationFrames.Num() == 0 || MainSequenceBuffer.Duration <= 0.0)
	{
		return false;
	}

	// Sequence has been consumed — playback time has passed the buffered duration.
	// Lock-free read: CurrentSequenceTimePassed and Duration are single-word values;
	// a stale snapshot is acceptable here since this only drives a blend-out decision.
	return CurrentSequenceTimePassed <= MainSequenceBuffer.Duration;
}
