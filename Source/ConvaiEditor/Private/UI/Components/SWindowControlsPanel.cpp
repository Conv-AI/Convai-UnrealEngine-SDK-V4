               ...
               .Padding(0)
                   [SNew(SOverlay) + SOverlay::Slot()[SAssignNew(SettingsHoverState->anchor, SMenuAnchor).Method(EPopupMethod::UseCurrentWindow).UseApplicationMenuStack(false).Placement(MenuPlacement_BelowRightAnchor).OnGetMenuContent_Lambda([this, SettingsHoverState]()
                                                          ...
                                                          .OnMenuOpenChanged_Lambda([this, SettingsHoverState](bool bOpen)
                                                             ...
                                                             Invalidate(EInvalidateWidget::Layout); })[SAssignNew(SettingsButton, SButton).ButtonStyle(FEditorStyle::Get(), "NoBorder").ContentPadding(SettingsButtonPadding).ToolTipText(LOCTEXT("SettingsTooltip", "Settings")).Cursor(EMouseCursor::Hand).OnClicked_Lambda([SettingsHoverState]()
                                                                                                                                                                                                                                                                                                                {
...
{
    // Cleanup all active menu detectors in one centralized call
...
bool FMenuClickOutsideDetector::HandleMouseButtonDownEvent(FSlateApplication &SlateApp, const FPointerEvent &MouseEvent)
{
    ...
    // Check if click is on the menu button/anchor itself
    FGeometry AnchorGeometry = Anchor->GetTickSpaceGeometry();
    if (AnchorGeometry.IsUnderLocation(CursorPos))
    {
        // Click is on the menu button, don't close (let it toggle or handle hover)
        return false;
    }

    // Use LocateWindowUnderMouse to get widget hierarchy under cursor
    ...
    {
        // Check if any widget in the path is our menu
        for (int32 i = 0; i < WidgetsUnderCursor.Widgets.Num(); ++i)
        {
            TSharedRef<SWidget> Widget = WidgetsUnderCursor.Widgets[i].Widget;

            // Check if this is the menu anchor
            if (Widget == Anchor)
            {
                // Click is on the menu anchor, don't close
                return false;
            }

            // If a specific menu widget type was provided, check for it
            if (!MenuWidgetTypeName.IsEmpty() && Widget->GetTypeAsString().Contains(MenuWidgetTypeName))
            {
                // Click is inside the specific menu widget, don't close
                return false;
            }
        }
    }
    ...
    // Don't consume the event, let it propagate to other handlers
    return false;
}
